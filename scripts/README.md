...
| `generate_tool_docs.py` | Writes `docs/dev/TOOL_REFERENCE.md`, the reader's reference to every registered tool (tier, identity class, the tool's timeout and a ceiling per action, workflow and older names, served description), from the runtime registry. `--check` exits 1 when the committed file is stale and is wired into the CI `smoke` gate. Exit 2 means it could not look (a missing dependency, or the handler package did not import); exit 3 means it read the registry but refused to publish it (a handler module failed to import, a router's action table could not be read, or no first-party tool registered). `make docs` runs it with the other generated-doc generators. |
