...
@contextmanager
def _checkpoint_transaction(path: Path) -> Iterator[None]:
    """Hold one nonblocking process lock for a complete bulk apply transaction."""
    ...
    try:
        ...
        raise UnsafeApplyError("could not open checkpoint transaction lock") from exc
    acquired = False
    try:
        if not stat.S_ISREG(os.fstat(descriptor).st_mode):
            raise UnsafeApplyError("checkpoint transaction lock is not a regular file")
        os.fchmod(descriptor, 0o600)
        try:
            fcntl.flock(descriptor, fcntl.LOCK_EX | fcntl.LOCK_NB)
        except BlockingIOError as exc:
            raise UnsafeApplyError("another surfacer bulk apply is already running") from exc
        acquired = True
        yield
    finally:
        if acquired:
...
def _existing_semantic_keys(finding: Mapping[str, Any]) -> tuple[str, ...]:
    """Include keys compatible with markerless titles that truncated the surface."""

    keys = list(semantic_keys(finding))
    title_surface = render_title(finding).removeprefix("dogfood friction:").strip()
    full_surface = str(finding.get("surface") or "").strip()
    if title_surface != full_surface:
        title_compatible = dict(finding)
        title_compatible["surface"] = title_surface
        keys.extend(semantic_keys(title_compatible))
...
class Surfacer:
    ...
    def _validate_apply_contract(self, boundary: datetime | None) -> None:
        ...
        if self.apply and not self.fingerprints and self.checkpoint_path is None:
            raise UnsafeApplyError(
                "unsafe --apply: bulk runs require a checkpoint file for locking"
            )
    ...
    def _existing(self) -> ExistingIndex:
        ...
        for search_term in search_terms:
            ...
            for issue in issues:
                ...
                identity = _issue_identity(normalized_issue)
                existing_issue = issues_by_identity.get(identity)
                if existing_issue is not None and existing_issue != normalized_issue:
                    raise RuntimeError("existing issue index returned inconsistent data")
                issues_by_identity[identity] = normalized_issue

        for issue in issues_by_identity.values():
            number = issue["number"]
            raw_comments = self.io["gh"](
                [
                    "api",
                    "--paginate",
                    "--slurp",
            ...
            try:
                pages = json.loads(raw_comments) if raw_comments else []
            except json.JSONDecodeError as exc:
                raise RuntimeError("could not read complete issue comments") from exc
            if not isinstance(pages, list) or any(
                not isinstance(page, list) for page in pages
            ):
                raise RuntimeError("could not read complete issue comments")
            comments: list[Issue] = []
            for page in pages:
                for comment in page:
    ...
    def run(self) -> int:
        """Execute under one checkpoint lock for the full bulk transaction."""

        if self.apply and not self.fingerprints:
            if self.checkpoint_path is None:
                if self.not_before is None:
                    raise UnsafeApplyError(
                        "unsafe --apply: provide --not-before and a checkpoint file "
                        "for a forward-only locked bulk run, or use --fingerprint"
                    )
                raise UnsafeApplyError(
                    "unsafe --apply: bulk runs require a checkpoint file for locking"
                )
            with _checkpoint_transaction(self.checkpoint_path):
                return self._run()
        return self._run()
    ...
    def _run(self) -> int:
        ...
        for f in self._collapse_batch(routed):
            ...
            if not fp:
                ...
                unresolved += 1
                continue
            keys = _existing_semantic_keys(f)
        ...
        if self.apply and not self.fingerprints and unresolved:
            raise UnsafeApplyError(
                f"bulk apply left {unresolved} unresolved eligible finding(s); "
