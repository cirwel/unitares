...
support inventory. The ablation lane contract is checked with synthetic/unit
tests; this guard must never query the live outcome-discrimination matrix.
Empty stdout means no regression detected.
"""
    ...
    *,
    ...
    try:
        run_repo_script(
            ...
            (
                "pytest",
                "tests/test_eisv_ablation_matrix.py::test_filter_rows_for_validation_excludes_beam_harness_by_default",
                "tests/test_eisv_ablation_matrix.py::test_split_rows_by_harness_lane_keeps_beam_visible",
                "tests/test_eisv_ablation_matrix.py::test_build_matrix_from_db_groups_lanes_and_respects_explicit_exclusions",
                "tests/test_eisv_ablation_matrix.py::test_parse_args_distinguishes_default_from_explicit_harness_exclusion",
                "-q",
            ),
            timeout_seconds=timeout_seconds,
        )
        evidence.append("ablation_lane_contract_tests=passed; live_outcomes_read=false")
    except (OSError, RuntimeError, subprocess.TimeoutExpired) as exc:
