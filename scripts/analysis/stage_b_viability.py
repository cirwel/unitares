...
def emit_auc(conn) -> None:
    ...
    residuals, phis, labels = [], [], []
    skipped = 0
    for is_bad, phi, beh in rows:
    ...
    print(f"  Direction on this sample: {lead}. CAVEATS: n_bad={n_bad} is far below")
    print("  inferential strength; labels are currently single-channel (Watcher")
    print("  adjudications) and effectively single-agent, so this is an instrument")
    print("  check, not a verdict on B. The gate needs breadth (more residents,")
    print("  more label channels) before either direction is claimable.")
