#!/usr/bin/env python3
"""Report outcome-attested prediction registry bindings.

This companion to the skeptical ablation matrix counts only persisted outcomes
that attest to a real prediction-registry binding, not fallback confidence or
audit bindings. Prediction creation time is not durable, so the report does not
classify these rows as prospective validation data.
"""
...
@dataclass(frozen=True)
class OutcomeAttestedCohortSummary:
    """Compact summary of outcome-attested prediction registry bindings."""
    ...
    def prediction_coverage(self) -> float:
        """Share of trusted rows attesting to registry-bound predictions."""

        return self.prediction_bound / self.total_outcomes if self.total_outcomes else 0.0

    @property
    def prediction_bound_bad_rate(self) -> float:
        """Bad-outcome rate within outcome-attested prediction bindings."""
...
# Backward-compatible symbol; temporal prospectiveness is not implied.
ProspectiveCohortSummary = OutcomeAttestedCohortSummary


@dataclass(frozen=True)
class ReadinessThresholds:
    """Minimum data-capture bar for outcome-attested prediction bindings."""
...
class CaptureReadiness:
    """Data-capture readiness decision plus explicit unmet gates."""
...
# Backward-compatible symbol; temporal validation readiness is not implied.
ValidationReadiness = CaptureReadiness


def is_registry_prediction_bound(row: OutcomeRow) -> bool:
...
# Backward-compatible symbol; temporal prospectiveness is not implied.
is_prospective_prediction_bound = is_registry_prediction_bound
...
def build_cohort_summary(
    ...
    *,
    ...
    lead_minutes: float,
    as_of: datetime | None = None,
) -> OutcomeAttestedCohortSummary:
    """Summarize outcome-attested registry bindings without fallback leakage."""
    ...
    registry_rows = [
        row for row in prediction_id_rows if is_registry_prediction_bound(row)
    ]
    ...
    return OutcomeAttestedCohortSummary(
        scope=scope,
...
def evaluate_readiness(
    summary: OutcomeAttestedCohortSummary,
    thresholds: ReadinessThresholds | None = None,
) -> CaptureReadiness:
    """Evaluate whether outcome-attested capture clears data-quality gates.

    This is a data-quality gate, not an EISV validation claim. It separates
    registry-bound volume, negative-class coverage, prediction coverage, and
    prior-state coverage so weak datasets fail explicitly.
    """
...
def format_cohort_report(
    ...
    *,
    ...
    """Render outcome-attested binding and capture-readiness evidence."""
    ...
    return "\n".join(
        [
            ...
            *funnel_lines,
            ...
            "",
            *readiness_lines,
            "Capture target: registry-bound prediction coverage for future holdout "
            "scoring; the outcome-attested rows above are capture diagnostics only.",
            "",
            ...
            "agent-state estimation (agent proprioception), not a grand jury and not "
            "an outcome oracle or bad-verdict dispenser; external labels still own "
            "outcome truth.",
        ]
    )
...
def build_cohort_contract(
    *,
    ...
    """Build a frozen outcome-attested cohort selection contract."""
