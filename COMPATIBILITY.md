...
|---|---:|---|
| UNITARES server | `v2.19.0` | Current supported runtime and canonical API behavior. |
| `unitares-governance` plugin | `v0.4.14` | Claude Code/Codex package release aligned with server `v2.19.0`. Its tagged skill bundle is a byte-identical mirror of this repository's `skills/` at the v2.19.0 release, verified against the published `SKILLS_MANIFEST.sha256`, and carries the coherence-semantics correction from plugin PR #116 and the v2.19.0 margin-semantics correction. Tested with Claude Code 2.1.220+ and Codex CLI 0.146.0+. |
| `unitares-sdk` | `0.2.1` | Published Python client for resident and custom integrations. `0.2.1` tolerates both `mcp` transport yield shapes (1.x's 3-tuple and 2.x's 2-tuple) as forward-compatibility groundwork; the `mcp` requirement stays `<2.0.0` because mcp 2.x expects an `httpx2` client where the SDK injects `httpx` (UDS transport path), which is a separate migration. Behavior is otherwise unchanged, so `0.1.0` callers keep working. Install the public release with `pip install unitares-sdk==0.2.1`; use the server's matching Git tag only when deliberately testing unreleased SDK changes. |
