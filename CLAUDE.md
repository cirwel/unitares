...
- **Delivery — autonomous queue by default:** every session lands its work as a draft PR, regardless of agent and regardless of whether the change is runtime code or docs/tests. `ship.sh`'s default `auto` route marks that completed draft for the resident merge conductor. While conductor execution or its required `agent-review` context is dormant, that marker is inert and a human maintainer remains the merge gate. After activation, the resident waits for strict CI, independent opposite-host review, clear surface claims, and a SHA-bound status before it readies or arms the PR. `--draft-pr` opens an unqueued/manual draft; `--direct` remains a docs/tests-only opt-out; `--auto-merge` is a deprecated alias for the same safe queued-draft path.
...
- **Merge authority is separate from authorship:** authoring agents do not ready or directly arm default queued PRs. The merge conductor does so only after its evidence gates pass, one PR at a time. Root/control surfaces (migrations, identity/auth, release/deploy authority, branch protection, delivery-contract files, and the conductor itself) remain manual unless `merge:root-approved` was applied through the separately configured root-approver GitHub App; a shared-account label is not authority. Verified root approval permits, but does not replace, two-agent review. `merge:hold` is the per-PR kill switch.
