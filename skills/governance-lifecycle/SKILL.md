...
4. Ordinary same-process check-ins: pass `client_session_id` (adapters may inject it); stateless `/mcp/` keeps no session binding between calls. Reserve `continuity_token` for explicit proof-owned rebinds.
...
- Session binding can happen via a transport session signal the client sends (an `X-Session-ID` header set per process, never a static value shared across processes; stateless `/mcp/` issues no protocol session of its own), `client_session_id`, or short-lived continuity token
