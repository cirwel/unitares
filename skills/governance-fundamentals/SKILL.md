...
- What that grounding is WORTH depends on who observed it. An outcome you submit yourself is graded `tool_observed` (evidence weight 0.65) at best, because the public path caps a caller attesting its own result — and 0.65 is exactly the calibration admission floor, so a self-described tool call is admitted at the boundary, not above it. Rows the server or an external signal observed can grade higher (0.85 / 1.00). The grade is not a score of your work; it is a statement about who checked. Below that cap, the response's `corroboration_hint` names the `detail` keys the grader actually reads.
