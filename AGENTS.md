...
For the human-facing Codex quickstart, see `docs/guides/CODEX_START.md`.
...
`docs/proposals/registered/eisv-outcome-grounding-stop-rule-v0.md` and the individuality-v2
...
- Redis — de-facto primary session/identity store, **not optional** (boots in degraded local-only mode without it, but most live sessions exist only in Redis; see `docs/proposals/archive/redis-retirement-v0.md`)
...
- **Active proposal/RFC docs in hot phase** — the Plexus / lease-plane / BEAM thread (`docs/proposals/active/plexus-scope.md`, `surface-lease-plane-v0.md`, `surface-lease-plane-phase-a-plan.md`, `beam-footprint-roadmap-v0.md`, `beam-coordination-kernel.md`). Restructure-during-flight is normal here; same rule as plan.md.
...
- **Reader-facing product prose** — `README.md`, `docs/PRODUCT_DEFINITION.md`, `docs/ROADMAP.md`. Not slot-based: the collision is semantic, two sessions rewriting neighbouring paragraphs from the same brief. Four PRs touched `README.md` over 2026-09-16/17, and two sessions worked the *same* recommended-rewrites table from `docs/ontology/competitive-analysis-2026-09.md` without either knowing: #2277 landed the A2A boundary and the record exports, then #2280 rebased onto them and discarded its own versions of both. A shared source document is the tell — if a table or audit lists the edits, assume someone else has the same list. The positioning lints (`PUBLIC_POSITIONING_CHECKS` in `scripts/diagnostics/check_doc_drift.py`, `_CONTESTED_CLAIMS` in `check_doc_health.py`) pin the product *sentence* and the retired wordings; they will not notice a duplicated paragraph or a competing phrasing of the same idea, so coordination is the only control that applies here.
- **The plugin skill mirror** — `unitares-governance-plugin/skills/`. Do not sync it by hand. `.github/workflows/plugin-skill-sync.yml` (every master push that touches `skills/`) and an operator's local deploy wrapper both run `scripts/dev/open-plugin-skill-sync-pr.sh`, which keeps exactly one sync PR open on `auto/plugin-skill-sync` and closes it once it is no longer needed. A hand-made sync PR races that one and conflicts on `SKILLS_MANIFEST.sha256`; two landed within twenty minutes on 2026-09-26. If one is truly unavoidable, title it `chore(skills): sync mirror …`: the automation steps aside for that title and no other.
...
- **The author owns review completion; the operator should not have to prompt.** After opening or updating a PR, run `./scripts/dev/review.sh` to start or join an independent code review. `ship.sh` does this in the foreground and returns findings to the working agent. With `git config review.native true`, it joins native Codex review and requests it once for drafts that automatic review missed; otherwise it uses local reviewers. Prefer the other model; a fresh reviewer session using the same model is a valid fallback. Read the result, fix and push or explain every rebuttal with `./scripts/dev/review.sh dispose <file>`, then re-run for changed diffs. Past the [round cap](docs/operations/github-workflow-conventions.md#round-cap) (three rounds, only P2s open), a changed diff gets one fix verification by the opt-in local verifier instead of a full run; its record says it did not review the new lines. Record an external code review with `./scripts/dev/review.sh record <file> --reviewer-name <who> --independent`; advisory consult output alone is not a completed review. Council/dialectic is optional escalation for substantial design choices or disagreement. Join CI, then mark your own PR ready. Starting a background reviewer is not completion. **Check `scripts/dev/review_providers.json` first: never request a provider it lists as disabled; while Codex is listed there, do not post `@codex review`.** Without local tooling, post `@codex review` on the PR when Codex is available and let the `review` check read the result (fix findings and request again), or run a fresh-context subagent or council review and render its record with `review.sh record <file> --independent --emit --reviewer-name <honest-name>`, then post the printed body verbatim through your GitHub connector (rebuttals: `review.sh dispose <file> --emit`); never hand-build a review or disposition record ([details](docs/operations/github-workflow-conventions.md#requesting-review-without-local-tooling)). Exit 2 means UNREVIEWED: report the exact blocker and next action, keep the draft, and never describe the outage as clean. The optional sweep covers quiet drafts as well as ready PRs; see [Review workflow](docs/operations/github-workflow-conventions.md#review-workflow).
...
- **Delivery — draft PR for everything:** every session lands its work as a draft PR, regardless of agent and regardless of whether the change is runtime code or docs/tests. A human maintainer is the merge gate. Do NOT direct-push to a shared branch. `ship.sh` enforces this: its default `auto` route opens a draft PR for every change (`--direct` opts out for docs/tests-only pushes; `--auto-merge` only when a maintainer explicitly asks).
- **Auto-merge is the maintainer's tool for not waiting, and an agent never arms it:** arming `--auto` says "this one is approved, land it when green," which is the merge decision — so it stays the maintainer's, exactly like merging. What changed is that an agent must not treat the absence of auto-merge as a reason to watch. `master` is `strict` and "always suggest updating pull request branches" is on, so once a maintainer arms a ready PR, GitHub updates its branch when the base moves and merges it when checks pass, with nobody touching the branch (confirmed end-to-end 2026-08-14, #1658). Until then a green PR simply waits.
...
The MCP SDK runs handlers inside an anyio task group; asyncpg and Redis run on asyncio. Awaiting either from a handler on the shared loop can hold connections across unrelated awaits, and a ~60× in-handler amplification on the governance-MCP request path (2026-05-04) was first read as that coupling. The attribution did not survive the experiment it was registered against: PR #350 took the 3221-await `load_metadata_async` loop off the request path, the floor closed to sub-200ms, and `docs/proposals/active/beam-footprint-roadmap-v0.md` (V0.2 RESOLUTION) records that floor as the await loop rather than the scheduler. Every floor raised since resolved Python-side as well (#354, #360, #361, #533), so diagnose a slow handler as sequential awaits in our own loops first. The mechanism now has no measured instance, which is not the same as absence: its signature is one of six wired coordination-failure sub-types, five have never fired, and the window behind the zero ran well below reference write load — a coverage state, so the four-state rule above applies before citing it in either direction. Whether to keep or retire the framing is an operator call the roadmap has deliberately left open; the rules below stand either way, because they are cheap and independent of it. The full account — measurements, the re-attribution, and the retired workaround patterns — lives in `docs/UNIFIED_ARCHITECTURE.md` under *asyncpg, Redis and the anyio scheduler*.
