...
- **The author owns review completion; the operator should not have to prompt.** After opening or updating a PR, run `./scripts/dev/review.sh` to start or join an independent review. `ship.sh` does this in the foreground and returns findings to the working agent. Read the result, fix and push or explain every rebuttal with `./scripts/dev/review.sh dispose <file>`, then re-run for changed diffs. Consult, council, or human reviews count via `./scripts/dev/review.sh record <file> --reviewer-name <who>`; do not repeat a review already recorded for this diff. Join CI, then mark your own PR ready. Starting a background reviewer is not completion. If review cannot finish, report the exact blocker and next action. The optional sweep covers quiet drafts as well as ready PRs; see [Review workflow](docs/operations/github-workflow-conventions.md#review-workflow).
