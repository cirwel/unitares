...
| HTTP write/decision endpoints | **Live** | `src/http_api.py` — `POST /v1/effect-veto`, `POST /v1/tools/call`, `POST /api/findings`, `POST /v1/substrate/observe`. The **dashboard UI** is read-mostly, not read-only: the Adjudication pane (`dashboard/redesign/sections/adjudication.js`) posts operator verdicts to `POST /v1/sentinel/adjudicate` via `dashboard/redesign/data.js`, and the Security pane (`dashboard/redesign/sections/security.js`) revokes passkeys and dashboard sessions. |
