...
| **Dialectic resolution receipt (`drr.v1`)** — deployment-countersigned resolution record | `src/dialectic_receipt.py`; `seal_resolution_for_persistence` in `src/mcp_handlers/dialectic/session.py`; verifier `scripts/client/verify_resolution_receipt.py` | Wired at every terminal `resolved` write; mints only when `UNITARES_AIC_SIGNING_KEY` is set, which it is not on the live process, so zero receipts exist. With no key the stored record and `Resolution.hash()` are unchanged. | **KEEP-DORMANT — wake condition: non-exportable custody for the attestation key (a same-UID plist secret is the only custody available today and the identity plan rules it out for attestation) AND a second principal with an independent channel to pin the public key.** Enabling before both means signing records that any co-tenant process could forge. Decision packet: `docs/proposals/dialectic-resolution-receipt-v0.md`. |
