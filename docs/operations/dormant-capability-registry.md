...
|---|---|---|---|
| Agent Orchestrator on BEAM (ephemeral spawn + lineage provisioning) | `elixir/agent_orchestrator/lib/.../agent_runner.ex` | **Not parked — corrected 2026-09-17; "nothing spawns through it yet" was already stale when this row was written, and "Wave-3b frontier" named the wrong wave (3b is the identity-middleware handler port).** Decision A of the 2026-06-24 Wave-3 gate closed **GO, and DONE**: first consumer named = the orchestrated independent dialectic reviewer, de-inerted and serving live reviews 2026-06-23 (#1020/#1021/#1024/#1025/#1029; live-verified own-identity reviewer submitting `agrees=false`). That consumer is `src/mcp_handlers/dialectic/orchestrator_dispatch.py`, called from `src/mcp_handlers/dialectic/handlers.py` and POSTing `/v1/agents`; `docs/operations/public-presentation-audit-2026-09-10.md` records session `e9bb8272f69786d7` completing through it. Dispatch is opt-in: `UNITARES_DIALECTIC_ORCHESTRATED_REVIEW` defaults to `0`, and any dispatch failure returns to the in-process synthetic reviewer, which stays both the default and the fallback — so a default install still spawns nothing through it. Provisioning shipped #581/#590/#648. | **BUILT + WIRED, DEFAULT-OFF** (same status the Theme 6 row for this flag already carries) — keep the tag so it survives cleanup; a zero on a default install measures the flag, not demand. Theme 8's orchestrator-vouched-identity row still carries "the BEAM Agent Orchestrator landing on a live surface" as its wake condition, and the orchestrator has landed in the opt-in configuration — that row is due its own read, not an automatic wake: `src/substrate/vouch.py` remains self-declared inert, and waking a credential seam is a separate decision with its own evidence bar. |
| Resident-validation authority/canary framework | `src/evaluation/resident_validation/{model,runner,invocation}.py` | Complete + tested; awaits a supervisor tick. Encodes governed-resident capability boundaries (forbidden deploy/merge/force-push) | **KEEP-DORMANT** — the authority model; do not cut |
...
| **Non-authored Phi cold-start authority guard** — prevent a substrate interpretation/observation or prediction from hard-pausing an identity solely on the non-discriminative fallback before authored/behaviorally ready evidence exists | `GOVERNANCE_NON_AUTHORED_COLD_START_GUARD=true` | **LIVE (default-on, stateless provenance guard)** | This is deliberately not the two-confirmation actuator above: it has no counter and cannot suppress behaviorally ready, independently verified, structural, CIRS, or loop-safety pauses. Since 2026-09-24 it also covers agent-authored reports (`GOVERNANCE_COLD_START_GUARD_INCLUDE_AUTHORED=true`, default; see the contract's 2026-09-24 amendment). Prospective trigger: the first 3/3 `shadow_would_defer` rows were `substrate_interpretation` and delivered circuit breakers; a fourth fresh-process reproduction auto-paused on the same epistemic class before its first authored check-in. Roll back on any audit showing the guard applied outside its exact provenance contract by setting the flag false and restarting. Reviewed recovery requires the persisted legacy circuit-breaker envelope and still gates on ownership, reflection, risk, and no-void; legacy `C(V)` is diagnostic context only. |
| **Reciprocal Claude delegation + Claude dialectic backend** — attributed Codex→Claude advisory calls and independently-accountable Claude review | `UNITARES_HOST_ADAPTER_ENABLED` (general tool); `UNITARES_DIALECTIC_ORCHESTRATED_REVIEW` + `UNITARES_DIALECTIC_REVIEWER_HOST=claude` (review) | **BUILT + WIRED, DEFAULT-OFF** | Wake only by explicit operator rollout after the authenticated CLI and orchestrator bearer are present, `list_inference_hosts` reports `claude:host-adapter` available and reachable from `delegate_inference`, and a safe-mode/no-tools/no-session-persistence canary returns exact `models_used` plus prompt/response hashes. For dialectic, also require the governed-spawn path and verify one session records the reviewer identity/model check-in and can preserve `agrees=false`. Roll back by removing the host/reviewer flags; local Ollama remains the fail-safe review backend. Subscription-backed usage is not asserted free: retain provider cost metadata. |
| **Codex delegation lane** — `delegate_inference(host_id="codex:host-adapter")` against the operator's ChatGPT-subscription Codex CLI | `UNITARES_HOST_ADAPTER_ENABLED` + `UNITARES_CODEX_CLI` (or codex on PATH) + `AGENT_ORCHESTRATOR_BEARER_TOKEN` | **BUILT + WIRED, DEFAULT-OFF** | Was `built_unwired` on the stated grounds that `invoke_host_adapter()` had no production caller. That stopped being true when `delegated_inference.py` was added — the service is host-agnostic — but the registry record and the `Literal["claude:host-adapter"]` in `DelegateInferenceParams` were never updated, so the reachability claim outlived its reason. Wake exactly like the Claude row: authenticated CLI plus orchestrator bearer, then confirm `list_inference_hosts` reports it reachable and a canary returns text with prompt/response hashes. The default model-aware path uses Codex app-server and records selected/effective model, provider, service tier, usage, CLI version, and explicit reroutes. `UNITARES_CODEX_APP_SERVER_INSTRUMENTATION=0` is the rollback switch to `codex exec --json`; that compatibility path cannot report a model identifier and therefore retains `models_used=[]` plus an explicit warning. The dialectic reviewer's own Codex lane (`UNITARES_DIALECTIC_REVIEWER_HOST=codex`, `agents/dialectic_reviewer/`) is a SEPARATE path that never went through this registry and was working throughout. |
| **Antigravity delegation lane** — `delegate_inference(host_id="antigravity:host-adapter")` and the third reciprocal `consult(effort="thorough")` peer, against the operator's Google-subscription Antigravity CLI (`agy`) | `UNITARES_HOST_ADAPTER_ENABLED` + `UNITARES_ANTIGRAVITY_CLI` (or agy on PATH) + `AGENT_ORCHESTRATOR_BEARER_TOKEN` | **BUILT + WIRED, DEFAULT-OFF** | Rides the same flag as the Claude and Codex rows. The orchestrator child runs `antigravity_cli_client.py`, which starts agy in plan mode and its sandbox from an empty temporary workspace with an allowlisted environment, and resumes a turn that stalled on a denied command (twice) or the output limit (once), the same handling as the PR review gate. agy reports usage but no model identifier, so `models_used=[]` with a warning. `consult` never routes a caller whose family it can detect (reported model or provider, harness, client, or user agent) to its own family, and takes the first *available* peer; an undetected caller, such as stdio or a `curl/8` or `python-httpx` user agent, may get any peer, Claude first, so `UNITARES_HOST_ADAPTER_DISABLED_HOSTS` (e.g. `codex`) is how a host-adapter lane whose provider account is unusable is taken out of `delegate_inference`/`consult` rotation; the dialectic reviewer's backend is chosen separately by `UNITARES_DIALECTIC_REVIEWER_HOST`. Wake: confirm `list_inference_hosts` reports it available, then a canary `delegate_inference` returns text with prompt/response hashes. |
| **Third-family dialectic reviewer** — an operator-configured OpenAI-compatible reviewer host (Gemini, an OpenAI endpoint, OpenRouter, a second Ollama) as a third model family beside Claude and Codex | `UNITARES_DIALECTIC_REVIEWER_HOST=external` (alias `gemini`) + `UNITARES_DIALECTIC_EXTERNAL_BASE_URL` / `_MODEL` / `_API_KEY_ENV` | **BUILT + WIRED, DEFAULT-OFF** | Deliberately vendor-NEUTRAL: #66/#80 removed a hardcoded `gemini` provider from `call_model` because nothing wired its key, so the branch could only return MISSING_CONFIG. Configuration selects the vendor here, so the path is exercisable with no key (proven live 2026-08-17 against `localhost:11434`: HTTP 200, `agrees=false` preserved, provenance carried the provider-reported model). Wake by setting base_url + model + a key env var. Before trusting it as an *independent* voice, verify the configured endpoint is not serving a Claude model — a correlated vote wearing an independence costume is worse than no third voice. Any failure (unconfigured, timeout, truncation, unparseable) degrades to the free local model, so an operator with no budget is never worse off. |
| **Antigravity as an interactive Gemini peer** — the desktop agent already holds a governance MCP connection to `:8767` | no flag; `~/.gemini/config/mcp_config.json` | **CONNECTED, NOT A HEADLESS PEER** | Verified 2026-08-17: the Antigravity `language_server` holds an ESTABLISHED socket to `[::1]:8767`, and its bundled `agentapi` CLI starts a conversation headlessly given `ANTIGRAVITY_LS_ADDRESS`, `ANTIGRAVITY_CSRF_TOKEN` (both readable from the running process) and `ANTIGRAVITY_PROJECT_ID` (from `~/.gemini/config/projects/*.json`). What it does **not** expose is the model's *answer*: `get-conversation-metadata` returns configuration, not transcript. So this path can fire-and-forget a prompt but cannot return a verdict — it is an interactive peer, not a reviewer backend, and it requires the desktop app to be running. Do not build a reviewer on it; use the external OpenAI-compatible host above. Empirical caution: every Gemini-family identity since 2025-12 (5 of them) produced exactly one check-in and never returned. |
| **Independent verification floor** — a deterministic, self-report-INDEPENDENT read of *described* adverse actions (audit/log tampering, backup destruction, credential/data exfiltration, concealment, protected-history rewriting, safety-control bypass), combined escalate-only so it can raise a verdict but never lower a worse Φ/drift signal | `GOVERNANCE_VERIFICATION_FLOOR=false` (apply) · `GOVERNANCE_VERIFICATION_FLOOR_SHADOW=true` (shadow, **default-ON**) · `GOVERNANCE_VERIFICATION_FLOOR_SHADOW_RECORD=all` (sink) / proposal `verification-weighted-verdict-v0.md`; Phase 2 wiring; Phase 2.5 sink `#2169` / PR `#2308` | **KEEP-DORMANT (apply); shadow wired AND recorded since #2169** | The gate is whatever `continuous-verdict-blending-v0.md` states for verdict-path changes — what that requires, one referent or two, is **open as D2** in `docs/proposals/active/open-decisions-packet-v0.md` — plus a false-positive-regression pass on a corpus larger than the bundled eval set. Only the corpus clause is now answerable. From Phase 2 until #2169 the shadow ran on every check-in of every default deployment and persisted **nowhere** — the firing was written onto the returned result dict and discarded with the process — so every "how often would it have fired" reading in that window was *not recorded*, never *zero*. `src/verification_floor_shadow.py` now writes each evaluation to `audit.events` under BOTH flag states (`applied` distinguishes shadow from enforced), including the non-firings, because the question is a rate and a numerator cannot answer it. Read it with `scripts/analysis/verification_floor_shadow_read.py`, which refuses to divide when a window carries no denominator and names the four states a zero can be in instead; `unitares_doctor`'s `verification_floor_shadow_recorded` warns when check-ins flow with no rows. **Two cautions before quoting any number from it.** (1) Read `strata.pronoun_free` before the headline rate: the detector reads English verb-object prose, and an earlier draft wrongly documented a first-person pronoun as *necessary* to fire and let the reader divide all firings by a first-person-only denominator — which reported a false-positive rate of 3.0 on the exact number this gate consumes. Input shape is a stratum, never an eligibility filter. (2) Self-report-independent is not substrate-independent: a caller whose `response_text` is a templated status line or a state digest is **unscored** by this channel, not cleared by it, so a clean record across a mixed fleet is not fleet-wide coverage. LIVE-AFFECTING: enabling the apply flag changes verdicts and can introduce pauses. Rollback is the flag; nothing to un-migrate. |
| **Baselined floor-breach verdict floor** — when an agent's behavioral baseline is warm AND an absolute EISV floor (`ABSOLUTE_E_FLOOR` / `_I_FLOOR` / `_S_CEILING` / `_V_CEILING`) is breached, the behavioral verdict is at least "caution"; risk, health and components untouched (a verdict floor, not a reweight) | `UNITARES_FLOOR_BREACH_CAUTION_SHADOW=` (shadow, default-off) · `UNITARES_FLOOR_BREACH_CAUTION_APPLY=` (apply, default-off) / issue `#1995`; counter `#2047` | **KEEP-DORMANT (apply); shadow available, not yet run** | The thread's own stop rule (recorded 2026-09-02) fired on its first condition 2026-09-13: the `#2047` counter recorded 59 **baselined** breach-and-safe rows (first 2026-09-12 11:54 MDT), which is the pre-registered trigger for building this candidate. Its as-written form ("any breach") is ruled out by the cold-start finding in the same thread — the harness's canned first three check-ins breach \|V\| for every fresh agent by arithmetic — so the gate on `is_baselined` is load-bearing, not optional. Wake the hands only after the standard shadow pass: run `UNITARES_FLOOR_BREACH_CAUTION_SHADOW=1`, read `payload.behavioral.absolute_floor_observation.floor_breach_caution` in `audit.events` (`would_change` is the numerator; every evaluated row carries the record so the denominator is complete), examine the actual rows the floor would have moved (the 09-12 cases are one agent at V ≈ −0.50 to −0.52 with behavioral risk 0.0002–0.16), and take an explicit operator decision. That decision also has to settle what the agent is told: as built, an applied floor raises the verdict only, so for the 09-12 shape the agent receives `guide` while its assessment reads `health: healthy`, `guidance: null` and risk near 0, and the decision reason reads "Proceeding mindfully"; the breach reaches the agent only as telemetry fields (`absolute_floor_observation.breached_dimensions`, its measurement snapshot, and the component such as `high_V`), not in guidance or the decision reason. Rows the floor changed carry `policy_effect: behavioral_verdict_raised`. The label says only that the floor raised the behavioral verdict. Whether the final decision changed is read from the same `auto_attest` audit row: `details.reason` names the deciding branch ("Proceeding mindfully" is the caution branch; pauses and the boundary-basin guide carry their own reasons) and `details.unitares_verdict` is the final verdict. It cannot settle three cases: Φ's own verdict under `phi_floor` is not kept, the CIRS soft-dampen and fast-trip paths are ambiguous, and a warmup pause the floor kept is identical in that row to one it did not keep (the grace runs after the row is written; only the absence of a `warmup_structural_suppressed` event shows it). The floor also has no hysteresis: it switches at exactly the ceiling (|V| > 0.50), where the 09-12 rows sit, so an agent oscillating across it flips safe/caution on every check-in; those flips feed CIRS, where 4 in a window of 10 set `resonant` (`trigger=flips`), hold the adaptive thresholds off their decay, and add `cirs_resonance` to a later pause's independent hard stops. Every resonant check-in also persists a server-attested `cirs_resonance` outcome with `is_bad=true`, so floor-induced flips would manufacture bad outcomes that feed outcome-grounded evaluation. Whether to add hysteresis is part of the enable decision. APPLY can also keep a pause, not only produce `guide`: the warmup structural grace lifts a cold-start void/coherence/basin/cirs pause only when the behavioral verdict is `safe`, and reads it whatever the verdict source (even with `GOVERNANCE_BEHAVIORAL_VERDICT` off), so after a restart a baselined agent at a floor breach keeps a pause it would otherwise have been spared. LIVE-AFFECTING when applied: behavioral is authoritative post-warmup under `UNITARES_PHI_TELEMETRY_ONLY`, and `monitor_decision` maps "caution" to `guide`. Rollback is the flag. |
...
| Capability | Location | Live evidence | Status |
|---|---|---|---|
| **Agent Identity Credential (AIC)** — third-party-verifiable attestation | `src/identity/agent_identity_credential.py` | **Not previously in this registry.** `UNITARES_AIC_SIGNING_KEY` is **unset on the live process** (PID 68291) and the module deliberately treats a missing key as "attestations disabled" rather than auto-generating (`:120-130`, correctly — an ephemeral key would silently invalidate every previously-issued AIC). **Zero non-self callers**: the only external reference is a comment in `src/effect_grant.py:24-37` stating it is *not* the AIC. Since 2026-09-02 one wired-but-dormant consumer signs with this key: the dialectic resolution receipt (next row). | **KEEP-DORMANT — wake condition: a consumer that needs third-party verification of an identity claim.** Do not read its zero usage as evidence about demand; it cannot be called. Note the ordering constraint this creates: any decision about what the AIC would attest (e.g. whether compute-derived trust tier 3 may be called "verified") is decidable *now*, precisely because nothing false has been signed yet. |
| **Dialectic resolution receipt (`drr.v1`)** — deployment-countersigned resolution record | `src/dialectic_receipt.py`; `seal_resolution_for_persistence` in `src/mcp_handlers/dialectic/session.py`; verifier `scripts/client/verify_resolution_receipt.py` | Wired at every terminal `resolved` write; mints only when both `UNITARES_DIALECTIC_RESOLUTION_RECEIPTS` (dedicated gate, default off) and `UNITARES_AIC_SIGNING_KEY` are set; neither is on the live process, so zero receipts exist. With issuance off the stored record and `Resolution.hash()` are unchanged; the receipt never alters the record it signs. | **KEEP-DORMANT — wake condition, all five: non-exportable custody for the attestation key (a same-UID plist secret is the only custody available today and the identity plan rules it out for attestation); an independent channel for a peer to pin the public key; retained and published key history; a revocation or transparency policy that bounds back-dating; and a second principal that exists and has a reason to check a record (none exists as of 2026-09-25).** Enabling before the first four means signing records that any co-tenant process could forge, or that a rotation or leak would orphan or back-date; enabling without the fifth demonstrates the code on the operator's own hardware, not the cross-principal trust model it exists for. Decision packet: `docs/proposals/active/dialectic-resolution-receipt-v0.md`. Decision record: `docs/proposals/active/federation-trust-decisions-2026-09-25.md` records multi-principal trust work as **not scheduled** (D3a), so the fifth condition has a recorded wake: a real outside party, a person or organisation with its own administrative root, asks to verify a record from this deployment. Custody for the key is deferred behind that wake (D3b); citing the record does not satisfy the non-exportable custody condition, and none of the five conditions changes. |
| **Dialectic review-request channel** — `request_review` / `dialectic(action="request")` | `resolve_dialectic_agent_id` in `src/mcp_handlers/dialectic/auth.py` | **Regressed into unreachability after the 2026-06-16 inventory**, which listed only dialectic *sub*-features (`check_reviewer_stuck`, quorum `ESCALATE`, calibration backfill) and assumed the channel itself worked. Dogfooded 2026-07-31: the auth check resolves the auto-injected `agent_id` (the **public structured handle**, documented `is_identity_key: false`) via `_agent_exists_in_postgres`, which expects a **uuid**. Auto-injection makes the working `if not provided:` branch unreachable for every session-bound caller. Leaves an unsubmittable orphan session (`allowed_agent_ids: []`). Issue **#1414**. | **FIXED — adoption unmeasured, and that is not a finding.** #1414 landed in `bf5ad3b4` (#1420), which resolves the caller by uuid instead of the public handle; `cee10ef9` (#2232) then enforced resolved callers across every transport. The prior "usage = 0, re-open needs a new premise" reading of this channel was measuring the bug, so it is void as evidence and must not be carried forward. A fresh count is possible now and none has been taken; until one is, the channel's usage is *not recorded*, which is a different state from unused and carries no authority to retire it. |
| **Constraint-pressure replay primitive** — pure leaky-integrator observer state over a named, operator-owned constraint residual (the PC-ALM-inspired "dual accumulator") | `src/constraint_pressure.py` (`ConstraintPressureConfig`, `ConstraintPressureState`, `advance_constraint_pressure`) | **Added 2026-09-16 by PR #2250 with no row here — the shape Rule 4 names, and blind spot (1) below: a capability added after the inventory.** The PR body places it "entirely outside the monitor, EISV, outcome, and policy paths" and the module docstring says the same. No static reference outside `tests/test_constraint_pressure.py` (16 invariant tests) across `src/`, `agents/`, `scripts/`, `governance_core/`, searched by module and class name; the module defines no `getattr`/`importlib` dispatch and no tool registers it, so the count is not missing a string-dispatch path. No flag, no shadow, no audit event, no MCP surface. The replay script the proposal's implementation seam names (`replay_constraint_pressure.py`, planned for the analysis scripts directory) does not exist on disk; the proposal records it as blocked on selecting an eligible real constraint. The test is coverage, not wiring: unlike `trust_contract_lint`, this primitive was built for a production consumer (the Phase 3 shadow) that does not yet exist. | **KEEP-DORMANT — wake condition: a separately approved, preregistered offline replay (proposal Phases 1–2) over an operator-selected constraint that has an owned reference and an independent resolution/outcome path; and only if that clears its predeclared bar, a default-off prospective shadow (Phase 3) that calls this same transition after the live decision is fixed and emits `constraint_pressure_shadow.v0` with `applied: false`.** Wiring it into `UNITARESMonitor`, EISV, risk, coherence, or any outcome producer before then is the double-counting / self-fulfilling-label failure the proposal names, not a wake. Do not read its zero usage as evidence about demand: it has no caller by construction. Decision packet: [`../proposals/active/pcalm-primal-dual-governance-v0.md`](../proposals/active/pcalm-primal-dual-governance-v0.md). |
