...
|---|---|---|---|
| **Autonomous merge execution** — serial PR review/arming by the resident merge conductor | `UNITARES_MERGE_CONDUCTOR_EXECUTE=0` (report-only default); `UNITARES_MERGE_CONDUCTOR_REVIEW=0` controls model calls during shadow | **KEEP-DORMANT (execution); deterministic shadow wired** | First provision root-attested, mutually distinct author/conductor/reviewer UIDs: a conductor LaunchDaemon and HOME, separate reviewer subscription HOME plus narrow root-deployed worker/sudoers rule, disjoint conductor credential root, isolated/no-site Python with an OS-Python preflight over every root-owned import root, root-owned deploy graph, pinned root-owned provider CLIs/runtimes/PATH, and the canonical `~/.local/state/git-surfaces` registry for each author UID (custom state-root overrides are unsupported). Pass author-writable `.pth`, claim-visibility, and negative author/reviewer credential canaries. Provision the resident fine-grained PAT with Administration read, Pull requests write, Contents read, and Checks read only; keep every admin/Contents-write credential out of the service, and canary the SHA-bound pull-request update endpoint. Then provision the review App distinct from the independently hosted root-approver App, require resolved review conversations, install the App-bound review check as OS root, provision the lease bearer, pass the two-process lease race, complete the 24h availability/armed-stall soak, and run both model families over identical evidence on Codex/Claude canaries. Confirm a competing post-Ready arm, final-snapshot authority mutation, final CI/thread/claim change, and newer failed root check are caught before explicitly setting EXECUTE=1. Rollback is EXECUTE=0, OS-root gate uninstall, and disabling any armed request. Root/control surfaces remain manual unless the separate root App owns both exact-SHA proofs and `agent-root-approval` is itself required and pinned to that App at the merge point. |
