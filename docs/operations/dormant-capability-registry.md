...
## Rule 4 entries — merged unwired, registered at merge (2026-09)

Not a sweep. Rule 4 says a new capability ships with one wired consumer *or* arrives
here naming what would wire it. These rows were written when the capability merged,
so the dormancy is declared rather than rediscovered later by a grep. That is the
blind spot Theme 7 names, closed at the point it opens.

| Capability | Location | Live evidence | Status |
|---|---|---|---|
| **Constraint pressure** — pure leaky-accumulator observer over a one-sided constraint residual, the PC-ALM-inspired replay primitive | `src/constraint_pressure.py`; tests `tests/test_constraint_pressure.py` | Merged deliberately unwired in #2250 (`7d88c183` 2026-09-15, merged `8cbea008` 2026-09-16). Zero callers outside its own test: a case-insensitive grep for `constraint_pressure` / `ConstraintPressure` across `src/`, `tests/`, `scripts/`, `config/` and `agents/` returns the module, `tests/test_constraint_pressure.py`, and prose in `docs/proposals/`. `src/__init__.py` re-exports nothing, and the one dynamic-import list (`_load_pydantic_schemas` in `src/tool_schemas.py`) is an explicit literal that does not name it. the offline replay script the proposal's seam list names at `:325` **has not been written** — the replay half of the design is unbuilt, so there is no offline consumer either. The module says so itself (`:1-6`: "deliberately has no governance-monitor, persistence, or policy integration"), and the PR's authority boundary states it "selects no real governance constraint, reads no outcomes, emits no audit event, and changes no live response or policy". | **KEEP-DORMANT — wake condition: a separately approved, preregistered replay (Phase 0 through Phase 2 of [`pcalm-primal-dual-governance-v0.md`](../proposals/pcalm-primal-dual-governance-v0.md)).** Only after that may Phase 3 add a default-off prospective shadow, which must compute *after* the live decision, run on a copy or pure input, and emit `constraint_pressure_shadow.v0` with `applied: false`. Do not read its absence of callers as evidence about demand: nothing can call it until the replay script exists, which is case (c) of the three states this registry's header names. Build the replay before reconsidering the row. |
