...
| **Dialectic resolution receipt (`drr.v1`)** — deployment-countersigned resolution record | `src/dialectic_receipt.py`; `seal_resolution_for_persistence` in `src/mcp_handlers/dialectic/session.py`; verifier `scripts/client/verify_resolution_receipt.py` | Wired at every terminal `resolved` write; mints only when both `UNITARES_DIALECTIC_RESOLUTION_RECEIPTS` (dedicated gate, default off) and `UNITARES_AIC_SIGNING_KEY` are set; neither is on the live process, so zero receipts exist. With issuance off the stored record and `Resolution.hash()` are unchanged; the receipt never alters the record it signs. | **KEEP-DORMANT — wake condition, all four: non-exportable custody for the attestation key (a same-UID plist secret is the only custody available today and the identity plan rules it out for attestation); an independent channel for a peer to pin the public key; retained and published key history; and a revocation or transparency policy that bounds back-dating.** Enabling before all four means signing records that any co-tenant process could forge, or that a rotation or leak would orphan or back-date. Decision packet: `docs/proposals/active/dialectic-resolution-receipt-v0.md`. |
