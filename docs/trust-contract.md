...
|5|`get_governance_metrics` auto-minted identity `auto_20260610_…` via ip/ua fingerprint on a read call|§3.5                            |High    |**CONFIRMED / NARROWED** — mint is real and per-call (3 cold probes → 3 distinct `auto_*` identities) but **in-memory only: zero `core.identities` rows written** (count stayed exactly 3,934 through all probes). The ghost-ROW correlation is refuted — the 3,934 population is historical (April-era `mcp_*` poller mints predating the reserved-prefix gate). The residual (per-call in-process monitor/metadata growth) is **CLOSED for the probed tool**: the mint lived in `require_agent_id` FALLBACK 2 (handler layer — the middleware already honored `requires_identity="pre_onboard"`), and `get_governance_metrics` now serves its unbound shape before reaching it on BOTH transports (PR #608): the MCP handler AND the REST direct-handler shortcut (`_DIRECT_HTTP_TOOL_HANDLERS`), which review caught bypassing the first fix entirely — the REST shortcut was the cold probes' actual transport. One shared payload, one guard per transport. Other tools auto-mint by design pending the #425 strict-identity rollout. The §7 read-purity experiment was run as designed and is what separated row-factory from in-memory reality|
...
|8|`get_governance_metrics` answers an **unknown** `agent_id` with the seed vector under a `success` envelope|§2, §3.5                    |High    |**FIXED (2026-08-10)** — row 5 closed the *unbound caller* case; this is the *unknown target* case, which that guard runs past. Any `agent_id` that resolves to no identity reached `get_or_create_monitor`, which builds a monitor on the default seed, so `E/I/S/V`, basin, verdict and guidance all shipped as functions of that seed while the envelope said success. Distinct from the §5 ignorance shape: "onboarded, no observations yet" is a real state honestly reported, but "no such agent" was reported as a *reading*. Discovered downstream, not by probe — the Discord governance HUD displayed 50 agents at a constant E=0.70 I=0.80 S=0.20 V=0.00 because it fed back the redacted display handles `list_agents` returns to non-operator callers (`Claude_Code_<date>_<uuid8>`), none of which resolve. Live DB check: 50/50 HUD agents matched a real identity by UUID prefix, 0/50 by the id being sent. Now refused with `error_type: unknown_agent` and no EISV keys, on both transports per the row-5 lesson|
...
- **Provenance linting.** A response-schema validator that rejects any emitted numeric/label field lacking a provenance class. Mechanical, not judgment-based.
  *Status: shipped for the `get_governance_metrics` read surface — `src/trust_contract_lint.py` + `tests/test_trust_contract_lint.py`. The linter is a declarative `LintSpec` applied mechanically; the governance-metrics spec encodes three rule families, each guarding a §6 violation class that was fixed but otherwise unpinned: **(A)** a field shipping with a provenance mechanism must keep it (`primary_eisv`→`primary_eisv_source`, `saturation_diagnostics`→inline `source`); **(B)** in an ignorance state no assessment field (`summary`, `state`, `stability`, `regime`, `phi`, `trajectory`, `mode`, `basin`) may carry a confident label — the generalized form of the single test this section names, subsuming rows 1–3; **(C)** fleet-scoped sub-blocks in an agent-scoped response must carry a `scope` key (row 4). The registry is deliberately the subset of fields that already have a provenance mechanism, not "every numeric field": extending it to a new field means wiring that field's provenance first, then registering it — never the reverse. `guidance` is excluded by design (an ignorance-state call-to-action is honest, and no mechanical check separates it from the row-2 "Pattern may be shifting" violation; that one stays pinned by `test_zero_observation_honesty`). Other endpoints (`observe`, `agent` status) reuse the engine via their own spec once their provenance is wired.*
...
- **Read-purity test.** Snapshot identity count, call every read endpoint unbound, assert identity count unchanged.
  *Status: CI assertions shipped for `get_governance_metrics` (PRs #608+#609, `tests/test_zero_observation_honesty.py` read-purity block — unbound call returns the unbound shape and injects no ghost agent_id; bound and explicit-id paths regression-guarded). The MCP-dispatch sweep found no other pre_onboard tool reaching the FALLBACK-2 generator; review then caught that the REST direct-handler table (`_DIRECT_HTTP_TOOL_HANDLERS`) is a SECOND dispatch surface needing its own sweep — its only other FALLBACK-2-capable entry was the same tool, now guarded; `health_check`'s direct handler never calls the generator. Lesson folded: any per-tool sweep must cover both dispatch tables. Full fleet-wide graduation (non-read tools) belongs to the #425 strict-identity rollout.*
...
- **2026-06-11** — PR #607 (saturation_diagnostics provenance label — external re-probe finding, labeled not suppressed) and PR #608 (read purity: unbound `get_governance_metrics` serves its ignorance shape without minting; the generator was handler-layer FALLBACK 2, not the middleware). §6 row 5 closed for the probed surface; row-1 family fully closed.
...
- **2026-08-10** — §6 row 8 opened and closed: an *unknown* `agent_id` was answered with the seed vector. Two things worth keeping from how it surfaced. **First, it was found by a consumer, not a probe.** Every §6 row above came from probing this server directly; this one came from noticing that a downstream HUD's numbers were too round and identical. A read that is wrong the same way every time looks like a calm system — the failure mode is quiet by construction, which is why it ran for weeks against a contract whose whole subject is exactly this. **Second, the closed row above it is what hid it.** Row 5 asks "did the caller prove who they are?"; nobody asked "does the agent being read exist?" — the guard fires on an unbound caller, then hands a bound caller's unresolvable target straight to `get_or_create_monitor`. Worth stating as a general shape: read-purity has two halves, *don't mint an identity for the caller* and *don't fabricate a reading for the target*, and closing one reads like closing both.
