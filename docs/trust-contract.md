...
- **Read-purity test.** Snapshot identity count, call every read endpoint unbound, assert identity count unchanged.
  *Status: CI assertions shipped for `get_governance_metrics` (PR #608, `tests/test_zero_observation_honesty.py` read-purity block — unbound call returns the unbound shape and injects no ghost agent_id; bound and explicit-id paths regression-guarded). The sweep found no other pre_onboard tool reaching the FALLBACK-2 generator. Full fleet-wide graduation (non-read tools) belongs to the #425 strict-identity rollout.*
