...
- `version: 1.16.0` is the negotiated interface release. Compatible additions
  ...
  the surface digest move; 1.16.0, 2026-09-25: the `skills` `name` description
  says a bare call returns an index without content, which is what the handler
  now returns; no parameter changes, and `skills`' input digest and the surface
  digest move).
