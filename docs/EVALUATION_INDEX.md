...
| `scripts/diagnostics/risk_authority_ablation.py` ✓ | Three-arm synthetic restart contract: resolved-low/Φ-high, resolved-high/Φ-low, and Φ-only | Machine-readable authority, persistence, and recovery checks; no live outcomes read and no live governance state mutated | current |
