...
|---|---|---|---|
| `docs/proposals/kg-agent-adoption-pilot-v0.md` + `scripts/eval/kg_agent_adoption.py` + `scripts/eval/run_kg_agent_adoption.py` + `scripts/dev/portable_knowledge_bundle.py` + `docs/evaluations/kg-agent-adoption/offline-fixture-canary-v0.receipt.json` | Seven-cell, chain-level test separating UNITARES KG value from information availability and surfacing; includes a neutral lexical substitute, complete result binding, and an exact/private offline exit drill | DRAFT — content-addressed offline-fixture canary passed under governed-review HOLD; enrollment remains unreviewed, and no cohort, live KG parity, production plumbing, scored model run, live alert, efficacy, preference, or portability claim is authorized | awaiting evidence-based scoped rereview, independent enrollment, live KG parity, and awaited recording canary |
