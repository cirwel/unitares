...
|---|---|---|---|
| `docs/proposals/orientation-constraint-set-preregistration-v0.md` | Paired, information-matched protocol for testing whether a read-only diagnostic constraint set improves source-justified next-action selection without becoming an authority or actuator | Frozen protocol candidate; no scenario enrolled and no treatment result read | awaiting enrollment |
