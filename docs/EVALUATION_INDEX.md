...
|---|---|---|---|
| `docs/proposals/kg-agent-adoption-pilot-v0.md` + `scripts/eval/kg_agent_adoption.py` + `scripts/eval/run_kg_agent_adoption.py` + `scripts/dev/portable_knowledge_bundle.py` | Seven-cell, chain-level test separating UNITARES KG value from information availability and surfacing; includes a neutral lexical substitute and an offline hash-verified exit drill | DRAFT — validation/scheduling/summary plumbing only; no cohort enrolled, no scored model run or live alert authorized, and no efficacy, preference, or portability claim | awaiting reviewed enrollment and recording-canary preflight |
