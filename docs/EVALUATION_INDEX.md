...
| `docs/proposals/eisv-incremental-value-ablation-v1.md` + `docs/evaluations/eisv-incremental-value/` + `scripts/analysis/eisv_incremental_value_contract.py` | Prospective, paired 12-arm test of EISV's incremental predictive and compression value over direct-evidence, behavioral, base-rate, and persistence baselines; includes an isolated dataset namespace, independent-label whitelist, temporal/group split contract, and fail-closed read receipts | DRAFT — schema/example/validator only; no cohort enrolled, no experiment scheduled, and no outcome read authorized | awaiting pilot enablement review |
