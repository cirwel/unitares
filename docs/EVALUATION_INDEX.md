...
|---|---|---|---|
| `docs/proposals/kg-agent-adoption-pilot-v0.md` + `scripts/eval/kg_agent_adoption.py` + `scripts/eval/run_kg_agent_adoption.py` + `scripts/eval/run_kg_agent_adoption_live_canary.py` + `scripts/dev/portable_knowledge_bundle.py` + `docs/evaluations/kg-agent-adoption/*canary*.json` | Seven-cell, chain-level test separating UNITARES KG value from information availability and surfacing; includes a neutral lexical substitute, complete result binding, exact/private offline exit drill, production-plugin KG probe, and one-row audit recording canary | DRAFT / HOLD — offline fixture independently reviewed; live root missed top five on five of six frozen queries and ranked fifth once; four source mappings remain non-byte-equivalent projections; one neutral audit row required read-only decoder recovery; a delayed auto-checkin then wrote state/outcome rows for the canary identity, falsifying durable isolation; no scored model, live alert, efficacy, preference, or production-plumbing claim is authorized | current 2026-08-27; blocked on independent live corpus/task enrollment, isolated canary identity/slot, first-read audit fidelity, full-MCP transport integrity, and orchestration reliability |
