...
- **`scripts/analysis/eisv_skeptic_report.py`** ✓ — computes AUC/Brier
  **lift of EISV/prior-state over a previous-outcome baseline**, emits a runtime verdict
  ...
  [resolved distributional-probe record](proposals/archive/eisv-distributional-signal-probe-v0.md)
...
read [`proposals/registered/eisv-outcome-grounding-stop-rule-v0.md`](proposals/registered/eisv-outcome-grounding-stop-rule-v0.md).**
...
|---|---|---|---|
| `docs/proposals/registered/accountable-coordination-ablation-v0.md` | Prospective four-arm, task-family-clustered comparison of independent work, ephemeral messages, durable coordination, and full accountable coordination; primary contrast is full UNITARES versus ordinary ephemeral coordination under matched models, tools, budgets, forced process discontinuity, external scoring, and a non-disableable outer experiment ledger | DRAFT — protocol only; no cohort enrolled, model calls scheduled, production toggle, protected read, policy change, or scored result authorized | independently reviewed 2026-09-15; awaiting task-family construction, planning-only power work, and a separate immutable enrollment |
| `docs/proposals/registered/resource-grounded-e-shadow-v0.md` + `scripts/analysis/resource_shadow_extract.py` + governance plugin `hooks/resource-observe` | Shadow test of whether observed context occupancy, added to current EISV, predicts near-term auto-compaction and tool failure better than EISV alone, the raw signal alone, and persistence; local-only opt-in collector, frozen hashed extracts, one read on held-out sessions (#2410) | Registered protocol only; collection is opt-in per machine and no extract or read has run. Estimand is operational-event prediction, never governance quality | new 2026-09-24; awaiting opt-in collection |
| `docs/proposals/active/kg-agent-adoption-pilot-v0.md` + `scripts/eval/kg_agent_adoption.py` + `scripts/eval/run_kg_agent_adoption.py` + `scripts/eval/run_kg_agent_adoption_live_canary.py` + `scripts/eval/run_kg_agent_adoption_live_canary_v1.py` + `scripts/dev/portable_knowledge_bundle.py` + `docs/evaluations/kg-agent-adoption/*canary*.json` | Seven-cell, chain-level test separating UNITARES KG value from information availability and surfacing; includes a neutral lexical substitute, complete result binding, exact/private offline exit drill, production-plugin KG probe, and one-row audit recording canary. The unexecuted v1 harness requires standalone loopback full-MCP capture and canonical event-ID JSON readback. | DRAFT / HOLD — offline fixture independently reviewed; live root missed top five on five of six frozen queries and ranked fifth once; four source mappings remain non-byte-equivalent projections; one neutral audit row required read-only decoder recovery; a delayed auto-checkin then wrote state/outcome rows for the canary identity, falsifying durable isolation; no scored model, live alert, efficacy, preference, or production-plumbing claim is authorized | current 2026-08-28; v1 harness hardened but unexecuted; blocked on independent live corpus/task enrollment, direct full-MCP availability, isolated quiet-period evidence, and orchestration reliability |
...
| `docs/evaluations/relational-calibration-instrument-supply/result-v1.md` + `docs/proposals/registered/relational-calibration-maturity-capacity-v1.md` | One-time frozen read of whether at least 200 recently active process identities carry temporally established, same-instrument, internally consistent EISV telemetry (instrument supply, not participant capacity) | `instrument_supply_not_ready`: `strict_supply = 0` (953 recent, 38 behaviorally mature, 0 temporally established); the stop rule closes the attempt; a disclosed pre-read row count was ruled harmless by the operator | current (2026-09-23) |
| `docs/proposals/eisv-incremental-value-ablation-v1.md` + `docs/evaluations/eisv-incremental-value/` + `scripts/analysis/eisv_incremental_value_{contract,pilot,federation,power}.py` | Prospective, paired 12-arm test of EISV's incremental predictive and compression value over direct-evidence, behavioral, base-rate, and persistence baselines; includes an isolated dataset namespace, independent-label whitelist, temporal/group split contract, fail-closed read receipts, a disabled-by-default immutable pilot store, signed score-free federation packages with cross-site independence linkage, and a planning-only power estimate | DRAFT — local and federated pilot infrastructure exists but checked-in collection is disabled; federation output has no arm scores or decision authority; no cohort enrolled, no experiment scheduled, no production outcome read authorized, and no confirmatory freeze claimed | awaiting pilot enablement and paired-power access review |
| `docs/proposals/registered/self-improvement-loop-evaluation-v0.md` | Three-arm preregistration separating operational closure, fixed automation, and adaptive learning; adaptive-versus-fixed held-out performance is the primary estimand | DRAFT - no cohort enrolled and no experiment scheduled | awaiting enrollment |
| `docs/proposals/registered/independent-operator-cohort-preregistration-v0.md` | Protocol for an external-operator deployment: usability lane (primary), predictive-validity lane via the shipped harness on the operator's own labels (gated), causal lane explicitly out of scope | DRAFT — registers at its PR merge; per-operator freeze at dated enrollment (#1607) | awaiting recruitment |
...
| `scripts/analysis/validate_theoretical_foundations.py` ✓ | Greps `governance_core` source for ODE/coherence terms vs theory doc | Pass/fail console | **⚠ brittle exact-string greps** — reads the in-repo `governance_core/` source directly |
...
[`proposals/registered/eisv-outcome-grounding-stop-rule-v0.md`](proposals/registered/eisv-outcome-grounding-stop-rule-v0.md).
