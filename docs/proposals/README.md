...
| [`verification-weighted-verdict-v0.md`](verification-weighted-verdict-v0.md) | v0 (2026-06-28) — Phase 1/1.5 landed (standalone): deterministic escalate-only detector (`governance_core/verification.py`) + local-model/Ollama backend (`src/verification_backend.py`, fallback-to-floor) + opt-in eval harness; separates the self-report-dependence worked example 0.0 vs 0.96. Phase 2 (actuator wiring, default-off flag) is **council-gated, not built.** Honors the one-sided Φ-floor constraint |
