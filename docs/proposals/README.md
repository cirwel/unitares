...
| [`outcome-fixture-conflation-decision-packet-v0.md`](outcome-fixture-conflation-decision-packet-v0.md) | **Decision packet (2026-09-02), awaiting operator selection.** Outcome rows posted without a caller-bound confidence are stamped `calibration_excluded`, and that flag is also a structural fixture marker, so every outcome-grounded instrument (ablation matrix, skeptic report, coherence dependency shadow, inventory) has dropped every trusted exogenous row since 2026-08-09 (1,005 of 1,005). Four options: fix the producers (registry-bound `prediction_id`), split the flags prospectively, split them retroactively with a disclosed deviation on the registered 2026-12-01 read, or nothing. Author's lean recorded; no code |
