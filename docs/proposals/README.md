...
| [`open-decisions-packet-v0.md`](open-decisions-packet-v0.md) | **Active** · **Decision packet (2026-09-09); the operator returned resolutions for all seven decision entries on 2026-09-10, none of them implemented.** Packages seven load-bearing calls as options + recommendation + reversibility/blast-radius + default-if-silent, ordered by what serves the project's future rather than by numbering: three that decay at the registered 2026-12-01 read (what PASS condition 4's "same family" means; whether to add an agent-stratified selective null; where to record the settled fact that condition 1 entails condition 2), three adopter-facing (is the published container image adopter-facing and is `dashboard/` inside the fleet-neutrality boundary; what the two-word review phrase names and what satisfies the `GOVERNANCE_VERIFICATION_FLOOR` enable-gate; which dormant doctor schema-drift checks run automatically and where), and one hygiene item (the tracked docs already tripping repo-scope Rule 5). Opens by establishing that no mechanism in the repo can decide these — `consult` takes only a text brief and creates no governance record, a dialectic is authored by an agent about its own recovery, and no delegation-recording mechanism exists in any Python or SQL file. The resolutions are reproduced verbatim in *Operator resolutions, 2026-09-10* and recorded as the operator's, not as decisions the packet made; implementing each is follow-up outside the packet PR, so every *Default if silent* clause still describes the present state. Separately, the packet's closing list of seven open evidence questions now carries operator-reported answers on three, from read-only checks on a deployment this repository cannot reach — the findings job is loaded and hourly and, by its own mode filter, runs none of the four drift checks, so its exit code says nothing about drift; registry, column and constraint checks pass; checksums cover 7 of 68 migrations, which is incomplete coverage and explicitly not drift. The other four evidence questions stand, and the column-check answer opens a new one, since PASS is emitted at any coverage down to zero resolved tables |
