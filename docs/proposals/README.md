...
| [`relay-substrate-relayering-v0.md`](relay-substrate-relayering-v0.md) | **Active** · Decision recorded (2026-09-17) on the 2026-09-16 packet — the NeMo Relay exporter and policy gate shipped as an optional SDK extra (`unitares_sdk.integrations.nemo_relay`); operator adopted the recommendation with the upstream-publication precondition inverted; new host integrations default to Relay and the hook chain is held; the execute-half parking is direction-only pending a design read, so nothing is parked or retired; does not reopen the Wave 3 go-decision; the ninety-day date is replaced by two named actions |
