...
| [`beam-verbs-as-contract-capabilities-v0.md`](beam-verbs-as-contract-capabilities-v0.md) | Draft v0 (2026-08-28), design-only, one adversarial review folded (mint assurance gate, `inbox` as consuming mutation, contract representability, `validate_path` scope) — which lease-plane verbs should become agent-facing capabilities in `unitares.interface-contract.v1` and which must not. Proposes `msg` yes / `lease` conditional / effects + force-release never. **Blocked on:** what "tool usage" means once BEAM verbs are capabilities (#1955 corruption precedent), the heartbeat-vs-acquire split, and `msg` ack/retry semantics |
