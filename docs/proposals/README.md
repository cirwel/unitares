...
Built 22 · Registered 7 · Active 26 · Parked 24 · Closed 14
...
| [`relay-substrate-relayering-v0.md`](relay-substrate-relayering-v0.md) | **Active** · DRAFT decision packet (2026-09-16) — the NeMo Relay exporter and policy gate shipped as an optional SDK extra (`unitares_sdk.integrations.nemo_relay`); three parking questions (host hook chain, governed-effect execute half, lease plane) stated for the operator with the evidence that would decide each; does not reopen the Wave 3 go-decision; ninety-day evidence gate named |
...
| [`pcalm-primal-dual-governance-v0.md`](pcalm-primal-dual-governance-v0.md) | **Built (dormant)** · Replay-first research design adapting PC-ALM's persistent-residual idea as non-authoritative constraint pressure; pure transition and synthetic invariants only, with no live wiring, selected real constraint, outcome read, or policy authority. The transition shipped in #2250 with zero callers outside its own test and the replay script unwritten; registered in the [dormant-capability registry](../operations/dormant-capability-registry.md) with its wake condition |
