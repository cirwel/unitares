...
2026-08-01. Rows posted with a confidence or a resolvable registered
prediction, or for which nothing could be scraped, are not stamped.
...
- **E1: distinct reasons at write, one gate for calibration.**
  ...
  `tests/test_outcome_events_scraped_confidence_exclusion.py`). It also changes
  item 2 of the legacy-coherence dependency shadow's fixed defaults
  (`legacy-coherence-dependency-ablation-v0.md`), a prospective contract
  registered on 2026-08-12 that has accrued no rows; that is disclosed in that
  document as an instrument correction before any read. A narrower variant,
