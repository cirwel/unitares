...
- §9 RFC test-name reconciliation — partial closure across two PRs. PR #291 shipped the audit script (`scripts/dev/audit_rfc_section_9_gates.py`); a follow-up added `# §9: <gate>` alias-annotation support to the script + 6 alias annotations on existing tests + 4 trivial gap-fill tests in `tests/test_lease_plane_section_9_gaps.py`. Audit progression: baseline 11 exact / 4 variant / 13 missing → post-reconciliation **21 exact / 1 variant / 6 missing**. Remaining 7 gaps (tracked as follow-up): 1 variant (`test_deprecation_sweep_uses_forced_release_reason` — needs explicit `release_reason='forced'` assertion); 3 Python missing (`test_file_canonicalization_case_insensitive_apfs` + `_relative_components` need FS-touching fixtures; `test_force_release_rejects_governance_token` needs broader force-release path test outside deprecation context); 3 Elixir missing (need `.exs` test file for HTTP router 409/200/surface_kind-rejection scenarios — no existing Elixir test file covers them as-named).
