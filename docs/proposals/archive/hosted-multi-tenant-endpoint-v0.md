...
| Remote auth on `/mcp` | **No global gate.** Default bind `127.0.0.1`; LAN exposure is opt-in env; the `/mcp` endpoint itself accepts unauthenticated POSTs. HTTP REST auth is optional and skipped on trusted networks | `src/mcp_listen_config.py:33-74`; `src/http_api.py:160-200`; `src/services/mcp_transport_service.py` (`_register_application_routes`, `make_streamable_mcp_asgi`) |
