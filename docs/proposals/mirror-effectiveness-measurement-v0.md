...
Phase 1 shipped: `scripts/dev/mirror_effectiveness_reeval.py` (Postgres or `--jsonl` truth-log) reads the emission log and emits a per-signal `effective` / `no_measurable_effect` / `insufficient_data` verdict from a surfaced-vs-shadow value-trend estimator, cohort-assigned by first-firing mode. **Implementation finding:** the threshold regression-discontinuity named below is *not* computable from Phase 0 as built — Phase 0 logs only *fired* check-ins, so the non-fired side of the `variance < 0.005` cutoff is absent. Realizing the RDD needs a Phase 0 extension that also logs near-threshold non-fired check-ins; the script reports the gap rather than faking it. Per-check-in complexity is also not in `core.agent_state` (migration 040), so the proposed agent_state join is unavailable and the emission log is the panel instead. Phase 2 (operator-gated feedback) remains proposed.
