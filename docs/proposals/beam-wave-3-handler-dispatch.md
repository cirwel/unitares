...
**Design choice surfaced (operator / next-revision call).** The conditions above implement **(A) fixed strongest-first lock ordering**. The architect lane recommends **(B): make the "reserve" saga INSERT itself the serialization point** — `idx_saga_one_pending_per_session` already enforces one-pending-per-session, so the unique-index conflict *is* the mutual exclusion; this removes FOR UPDATE from invariant 5 entirely (one lock mode on the row, no FK conflict) and aligns the primitive with the saga FSM rather than bolting a second, conflicting one beside it. (B) is the cleaner ontology; (A) is the smaller change. **Not yet decided.**
