...
either way — the cited artifact is in neither `master` nor any branch.
⛔**Corrected 2026-08-22.** The worksheet is named at `:305`, not `:210` (`:210` is the V0.4
historical status line and names no worksheet), and it is
`docs/handoffs/wave-3-state-ownership-redteam-**prep**-2026-06-28.md` — which v0.3.5 explicitly
calls *prep*, not the red-team. (D)'s named artifact is `wave-3-state-ownership-redteam-<date>.md`.
**Committing the prep worksheet therefore removes NO halt clause**; it supplies evidence toward the
adjudication only.
...
`docs/proposals/active/wave-3-reduced-scope-gate-v0.md` supplies a reduced disconfirmer set, rederived
...
engages only when that tree **exists**. The slice lands in the **existing**
...
⚠️ **Read this scope limit first.** What follows retires **a cut, not the wave.** Wave 3's goal —
BEAM owning coordination and state ownership — is committed and unretracted; only the operator
retracts it. ⛔This section is not licence to file Wave 3 as settled, to read "not built" as "not
wanted", or to demand a fresh justification before anyone resumes. Demand-gating a standing
commitment is circular: it measures the absence of the unbuilt thing.

⛔ **Weigh the (γ) evidence for what it is.** The retirement below rests substantially on a
latency/cost comparison (~78ms of movable locked compute against a ~1031ms enrichment tail). Latency
was explicitly **struck as a Wave-3 decision axis** by the operator on 2026-06-24 and again in the
V0.4 resolution; the live axis is *stop accumulating workarounds*, plus coordination and aliveness.
So the cost argument is a sound reason this **particular** cut was a poor target, and an unsound
reason to conclude anything about the wave. The structural findings (in-process singletons, the
inverted atomic seam, the identity FK) are the durable part and constrain *where* a future cut goes.
...
- **The design** — the (γ) narrow cut, the only shape carried to implementation — was **shelved three days later** on live telemetry, not on preference. Full record: [`beam-wave-3-gamma-hybrid-v0.md`](../archive/beam-wave-3-gamma-hybrid-v0.md) §0c.
...
**What this does not cover.** Wave 3a (read-only handlers) is a separate and completed wave — see [`resolved/beam-wave-3a-read-only-handlers.md`](../archive/beam-wave-3a-read-only-handlers.md); its handlers serve only argless/`tool_name`-only calls, arg-bearing calls delegate back to Python by design. Dialectic-on-BEAM shipped as a pure-compute/coordination port and is unaffected. Neither is evidence that this RFC's write-path port advanced.

**On resuming.** No new premise is owed — the commitment already is the premise. The useful history is that this RFC went through four redrafts against an *unchanged* substrate argument, which is a reason to bring a **different cut** rather than a fifth redraft of the same one. The §0c structural findings are the map of where not to cut; they do not close the territory.
...
> **Partly executed; see V0.5 above — the sequencing is paused, not cancelled.** Step 1's additive infra landed (`audit.coordination_measurements`, the `core.identities_shadow` / `core.agents_shadow` comparator tables, the saga DDL). **Step 2 never ran** — both shadow tables hold zero rows, so no divergence window was ever opened and Python remains sole writer. Step 3 was never reached. Kept as the design record; a resumption may reuse or replace it.
...
**Parent:** `docs/proposals/active/beam-footprint-roadmap-v0.md` v0.4.
**Sibling, completed:** `docs/proposals/archive/beam-wave-1-sentinel.md` (Surface 1+2 shipped; Surface 3 in flight).
**Sibling, completed:** `docs/proposals/active/surface-lease-plane-v0.md` Phase A + Wave 2 hardening + resident Phase B + lease RPC recorder/persistence (#412/#414/#417/#418/#419/#476/#480/#481).
...
**(E) Opportunity cost.** Per the operator's 2026-08-29 reconciliation, a projected calendar-week slip on either named item — {paper deadline, fellowship application} — exceeding 25% of its original deadline window is a **standalone halt**, regardless of projected Wave 3 duration. **No acceptance-memo escape hatch.** Re-opening requires re-scoping, not a written acceptance of the slip. The decision artifact must still state projected Wave 3 elapsed and compare it with the `Wave 1 elapsed × 3` cap; that comparison remains required reporting and may compel rescoping, but exceeding the cap is not, by this ruling alone, an independent halt. Wave 1's elapsed time is concretely named in §14 (currently estimated ~3 weeks; pinned to actual at gate). **Measurement source:** `docs/proposals/wave-3-go-decision-<date>.md` artifact written by operator at gate, with §"Calendar reasoning" enumerating each named item's current slip vs original target and the projected-elapsed cap comparison. (Was (F) in v0.2; named set amended 2026-08-22.)

**(F) Dialectic-quality regression.** During canary, dialectic session-resolution rate (resolved / (resolved + failed + escalated) over a 14-day window) regresses >5% against pre-Wave-3 baseline. Reviewer-reassignment rate increases >20%. **Measurement source:** baseline computed from trailing 30 days of `core.dialectic_sessions` rows (47 total as of 2026-05-09; gate halts on insufficient baseline volume if 30-day window has <30 sessions). Both baseline mean and σ pinned in §11 prior to implementation start (prereq PR #9). (Was (G) in v0.2.) **The rate MUST be computed through `DialecticDB.get_outcome_breakdown()` / `src/dialectic_outcomes.py`, never off raw `status` — see §11 criterion 10 and issue #1689 for what raw `status` counts as a failure.**

    ⛔**The `+ escalated` term in this denominator is DEAD — resolved by measurement 2026-08-23, not by ruling.** §11 criterion 10 and `dialectic_outcomes.py::resolution_rate` use `resolved + failed` with no escalated term, and this line disagreed with them. It does not matter: `escalated` is **council-retired and never written** (`src/dialectic_db.py:261` — "storable but council-retired"), and `core.dialectic_sessions` holds **zero** such rows all-time (status distribution is `failed` 85 / `resolved` 69, nothing else). ⛔The two definitions therefore agree in practice and will keep agreeing unless `escalated` is revived — at which point this term must be re-decided rather than inherited.

    ⚠️**Latent, flagged not fixed: three definitions of "terminal" disagree in code.** `dialectic_outcomes.py:41` = `{resolved, failed, timeout, abandoned}`; `dialectic_saga.ex:114` `@terminal_statuses` = `{resolved, failed}`; the saga's guarded-UPDATE `WHERE` = `{resolved, failed, escalated}`. ⛔A `timeout` or `abandoned` row is terminal to Python's classifier but **writable** by BEAM's guard — the same dual-writer hole #1804 closed for `resolved`/`failed`, still open for the other two. Currently unreachable (zero such rows ever), so it is recorded rather than fixed; ⛔reconcile the three before any status beyond `resolved`/`failed` is written.
...
3. **Mandatory gate (blocking).** `update_session_phase` / `update_session_status` / `resolve_session` (`dialectic_db.py:196/215/226`) currently mutate the row in bare autocommit with no gate; a FOR UPDATE waiter cannot see an ungated writer, so the invariant is only as strong as its least-disciplined write path. *[2026-08-21: all three now carry a `status NOT IN ('resolved','failed')` predicate; `update_session_awaiting_facilitation` and `add_message`'s `updated_at` touch remain bare. Predicate guards narrow the window; they are not the (B) serialization point and do not close this condition.]*
...
9. **(E / opportunity cost gate)** operator's `docs/proposals/wave-3-go-decision-<date>.md` includes §"Calendar reasoning" naming current slip vs original target on each of {paper, fellowship}; no item slips >25% of original deadline window. **No acceptance-memo escape.** Wave 1 elapsed time concretely named in the document; (E)'s "× 3" cap derives from the actual measured Wave 1 elapsed.
    **AMENDED 2026-08-22 (operator).** The named set was previously {paper, fellowship, HLH, R2 Phase 2}. Two of those four were never calendar items, so the criterion was not computable as written — and a decision artifact could satisfy it only by fabricating cells or by declaring them exempt, the latter being a gate that clears itself. Both are removed, for stated reasons:
    - **HLH — REMOVED.** No original deadline window was ever recorded for this track, and the event produced no outcome (Hugging Face opened registration; no leaderboard, results or finals exist publicly as of 2026-08-22, and its signup page still advertises its launch in future tense). Local work stopped 2026-05-07. ⛔It cannot slip, and its opportunity cost against operator calendar is **zero because the track is dead** — not because a number was unavailable.
    - **R2 Phase 2 — REMOVED from the calendar set; restated in its own terms.** It is **evidence-gated** (≥4 weeks telemetry, ≥50 promoted, ≥10 demoted, ≥1 cross-role-rejected — `docs/ontology/plan.md`), never date-gated, and correctly deferred on sparsity since the 2026-06-11 triage. ⛔It has no deadline window to slip against and consumes no operator calendar while deferred; its gate is upstream of any operator time. It is not an (E) input.
    - **NEW RULE — missing or inapplicable windows.** An item in the named set with no recorded original deadline window does **not** silently pass, and must **not** be recorded as "not computable" inside a decision artifact. It must be either (a) removed from the set by amendment to this criterion, with the reason stated as above, or (b) given a written original window before the gate is read. ⛔An undefined cell may never be treated as a non-halting pass.
    ✅**RECONCILED 2026-08-29 (operator): the standalone slip halt governs.** A projected slip >25% of either named item's original deadline window halts Wave 3 regardless of projected Wave 3 duration. The v0.3 fold stated that rule explicitly in response to an architect BLOCK; the stale conjunction in the former §0(E) wording cannot let a shorter implementation excuse a sacrificed item. The decision artifact must still state projected elapsed and compare it with the `Wave 1 elapsed × 3` cap as required reporting and a rescoping signal. ⛔This ruling does not make an over-cap projection an independent halt; that would require a separate amendment. Neither the cap nor an acceptance memorandum can waive the named-item halt. Reduced-scope gate §6.3 records the same ruling.
10. **(F / dialectic-quality gate)** session-resolution rate regression ≤5% AND reviewer-reassignment rate increase ≤20% vs baseline. Baseline (mean + σ) computed from trailing 30 days of `core.dialectic_sessions`; pinned in this §11 prior to implementation start (prereq PR #9). Gate halts if baseline volume insufficient (<30 sessions in window).
    **Pin (prereq PR #9, 2026-06-11 — `docs/handoffs/wave-3-dialectic-baseline-2026-06-11.md`):** trailing 30-day window held **1 session** → **(F) halts on its own volume haltspec; no 30-day mean/σ pinnable.** Context: 90-day n=9 → resolution 0.556 (binomial σ≈0.166); all-time n=48 → 0.646; monthly volume collapsed ~20/mo (2025-12) → ~1/mo (2026-05), coupled to the dialectic-rework arc — volume recovery is upstream of this gate. **Second-metric source created:** reviewer-reassignment rate previously had NO event stream (zero `%reassign%` rows in `audit.events` all-time; transcript-only — a violation of §0's source-naming discipline). `_apply_reviewer_reassignment` now emits `dialectic_reviewer_reassigned`; the reassignment baseline accrues from that deploy forward. ⛔**CORRECTED 2026-08-22 — it is NOT the single chokepoint, and the reassignment baseline is therefore incomplete.** `_apply_reviewer_reassignment` (`src/mcp_handlers/dialectic/handlers.py:933`) emits the event at `:1021`, but the periodic sweeper `auto_resolve_stuck_sessions` (`src/mcp_handlers/dialectic/auto_resolve.py:102`) writes reviewer changes directly through `update_session_reviewer_async` (`:197`) and calls **neither `_apply_reviewer_reassignment` nor `check_reviewer_stuck` nor `check_timeout`** — it emits nothing. Every auto-path reassignment is absent from the stream this baseline is computed on. ⛔Route the sweeper through the chokepoint, or emit the same event from it, and restart the reassignment window before pinning; the window accrued to date cannot support the ≤20%-increase test. ⛔**MEASURED 2026-08-22 — THE REASSIGNMENT METRIC HAS NO DATA AND NO PROSPECT OF DATA AT CURRENT RATES.** `audit.events` holds **zero** `dialectic_reviewer_reassigned` rows all-time, against 4,738,908 events since 2026-03-15. The entire dialectic history contains **two** reviewer reassignments — 2026-04-19 and 2026-04-30 — both predating the emitter's own 2026-06-11 deploy, across 153 sessions (141 with a reviewer). The sweeper has performed none. ⛔**A "≤20% increase vs baseline" test cannot be computed against a baseline of zero events**, so this half of criterion 10 is not merely under-volume like the resolution-rate half — it is unpinnable in principle until reassignments actually occur. ⛔Do not treat waiting for n=30 sessions as sufficient for this metric; the smaller gate must either supply a different reassignment measure or state that (F) rests on the resolution-rate half alone. **EMISSION GAP CLOSED IN CODE 2026-08-22 — ⛔NOT YET IN THE STREAM:** both producers now emit through `src/mcp_handlers/dialectic/events.py::emit_reviewer_reassigned`, tagged `source="request"` / `source="sweeper"`. ⛔The two paths were NOT merged — their write sequences differ and routing one through the other would be a behavioural change wearing an observability change's clothes; only the event *shape* is shared. ⛔**The clean reassignment window starts at the DEPLOY of that change — not at its merge, and not at the 2026-06-11 emission.** ⛔**No deploy timestamp is recorded here because none exists yet, and none is captured automatically:** verified 2026-08-22 that the `governance` database has no deploy table and no deploy-completion event type, `deploy-apply.sh` is human-triggered with no cron or launchd automation, and process-start time is not a proxy (the service is `KeepAlive`-restarted independently of deploys). ⛔**Whoever deploys this must record the wall-clock timestamp and the deployed commit HERE, and state the denominator-filter predicate, before criterion 10's reassignment half is pinned.** Until that line exists, no reassignment window has started and none may be cited. Every `dialectic_reviewer_reassigned` row predating it — there are currently zero — would be missing all auto-path reassignments. Re-pin when any trailing-30d window holds ≥30 sessions.
    **Re-pin trigger fired, and the gate still halts (2026-08-19, issue #1689).** The trailing-30d window now holds **47** sessions, so the volume condition as originally written is met. It must not be pinned on that number. `status` conflates three endings, and the two it wrongly counts as failures dominate this window: canary probes, which end `failed` by design, and sessions holding a standing reviewer objection that nobody facilitated, which is the dialectic working as intended. `awaiting_facilitation` already discriminates the third case exactly, so this is a reader fix, not a write-path change — `src/dialectic_outcomes.py` is now the single classifier and `DialecticDB.get_outcome_breakdown()` the reader.
    Measured 2026-08-19, trailing 30d, n=47: **canary 22** (excluded via `core.agents.label LIKE 'canary_dialectic%'`; `trigger_source` is the literal `'manual'` on every row and partitions nothing), **unresolved-awaiting-facilitation 14**, **resolved 11**, **failed 0**. Raw `status` rate would be 38.3%; the honest rate is 11/11 over a denominator of **11**. That denominator is **below the ≥30 floor**, so (F) continues to halt on its own volume haltspec — the same verdict as the 2026-06-11 pin, now for the correct reason. A 38.3% pin would have measured canary volume, not dialectic quality, and would have penalised the channel for producing real disagreement. **Do not re-pin until the resolved+failed denominator alone reaches 30.**
