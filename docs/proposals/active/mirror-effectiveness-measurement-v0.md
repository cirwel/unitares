...
Status: **Phases 0–1 landed** (Phase 2 proposed). Follow-up from the 2026-06-14 mirror-mode scoping (PR #741, which closed the #583 "reflect, don't advise" seam in `_detect_gaming`). Defines a deterministic, operator-funded-free way to answer "does a surfaced mirror signal actually change agent behavior?" — replacing the current dogfood-only, qualitative tuning loop. **Recommends instrumentation + offline analysis first; no agent-facing behavior change in v0.**
**Demotion review (2026-08-16):** Retain as the current measurement contract because operator-gated Phase 2 consumes the Phase 0–1 instrumentation and estimator definitions recorded here.

Phase 0 shipped: `enrich_mirror_signals` now produces structured `_mirror_signal_records` (autopilot complexity/confidence variance + complexity divergence) and `response_formatter._emit_mirror_signal_records` logs one `mirror_signal.emit` audit event per fired check-in, tagged `surfaced = (response_mode == "mirror")`. Off-switch: `UNITARES_MIRROR_SIGNAL_EMIT=0`.

Phase 1 shipped: `scripts/dev/mirror_effectiveness_reeval.py` (Postgres or `--jsonl` truth-log) reads the emission log and emits a per-signal `effective` / `no_measurable_effect` / `insufficient_data` verdict from a surfaced-vs-shadow value-trend estimator, cohort-assigned by first-firing mode. Per-check-in complexity is not in `core.agent_state` (migration 040), so the proposed agent_state join is unavailable and the emission log is the panel instead.

Phase 0.5 shipped: `_detect_gaming` now tags every record with `fired` and additionally logs the just-above-threshold **non-fired** band (`[0.005, 0.010)` variance) as `fired=False` control points — emit-only, no prose, no agent-facing change. This unlocks the **threshold discontinuity** (RDD-flavored) second estimator in the re-eval script: agents whose first near-threshold check-in fired (treated) vs did not (control) are compared on subsequent value trend, mode-independently. It is a mean-difference contrast at the cutoff, not a local-linear fit — it corroborates surfaced-vs-shadow, removing that estimator's mode-selection-bias as the sole basis. Phase 2 (operator-gated feedback) remains proposed.
