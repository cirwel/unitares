...
related:
  ...
  - docs/proposals/archive/path1-sync-fingerprint-check.md (precedent for log->strict phased rollout)
  ...
  - **docs/proposals/active/plexus-scope.md** (`Plexus` is the product/system boundary name for this RFC's coordination layer. Defines v1 ownership, non-goals, stop signs, and the manual `Plexus Zero` fallback protocol for service outages or callers that have not integrated the service yet. Does not redefine schema, API, or `surface_id` semantics — those remain here.)
unblocks: |
