...
**Shape:** follows the contract in `docs/proposals/archive/operator-decision-packet-v0.md`
...
1. **`consult` has no repository access and confers nothing by construction.**
   ...
   authority side, `docs/proposals/active/consult-advisory-facade-v1.md` states the
...
2. **A dialectic can only be authored by an agent about its own recovery.** The
   ...
   `docs/proposals/archive/operator-decision-packet-v0.md` finding 1 recorded `ESCALATE`
...
3. **The repo has no mechanism for recording a delegation.** `git grep` for
   ...
   (`docs/proposals/archive/operator-decision-packet-v0.md`,
   `docs/proposals/archive/track-b-operator-delegate-design.md`) and **zero** Python and
   **zero** SQL files. The one architecture decision on the subject,
   `docs/proposals/archive/ADR-001-operator-vision-delegation.md`, carries the status
...
make it. Per `docs/proposals/archive/operator-decision-packet-v0.md`: "The bottleneck
...
`docs/proposals/registered/eisv-outcome-grounding-stop-rule-v0.md` PASS condition 4 reads:
...
- **The sibling pre-registration reads "family" as the whole candidate set.**
  `docs/proposals/registered/independent-operator-cohort-preregistration-v0.md`: "The
...
- **Amending the sibling is currently free.**
  `docs/proposals/active/independent-operator-cohort-enrollments.md` records no
...
annotation to `docs/proposals/registered/eisv-outcome-grounding-stop-rule-v0.md`, in the
...
In `docs/proposals/archive/continuous-verdict-blending-v0.md`, one phrase names two
...
satisfies the still-open gate that `docs/proposals/active/verification-weighted-verdict-v0.md`
...
- **Repo-wide usage is near-unanimous that the phrase names a parallel subagent
  ...
  phrase, in `docs/proposals/archive/thread-trajectory-stitching-v0.md` — so "sole
  outlier" is true of the phrase and not quite true of the word.
- **A dated definition predates both lines by nine days.**
  `docs/proposals/archive/operator-decision-packet-v0.md` (2026-06-17), finding 4: the
...
- **The ambiguity already reaches a shipped actuator.**
  `docs/proposals/active/verification-weighted-verdict-v0.md` makes enabling
  ...
  `docs/proposals/archive/demotion-review-2026-08-16.md` re-affirms that
...
| **B** | **Two gates, two names.** Keep the phrase for the subagent review; rename the prospective gate to operator sign-off, in this file, in the sibling, in `docs/proposals/archive/demotion-review-2026-08-16.md`, and in the `docs/proposals/README.md` status rows. | Both mechanisms survive with distinct names, and the enable-gate on the live flag is unambiguously a human act. | Four files plus two index rows — the largest edit surface here. Each edited file trips the guard, so the PR must also settle strip-or-allowlist for each — a cost that exists only while the guard fires on the next touch, so under either form of entry 7's option E it drops and B's surface is four files plus two index rows. It converts a soft pending item into a named, visible commitment on one person's queue. |
...
- **Archive rather than allowlist.** `docs/proposals/README.md` marks this file
  parked since 2026-06-27; moving it to `docs/proposals/archive/` and
...
- **Bounded by running the guard over every tracked file: 23 violations in 22
  ...
  is an operator-home-path hit, in `docs/proposals/active/surface-lease-plane-v0.md`,
...
- **Seven of the 22 sit in `docs/proposals/archive/`, the repo's own shipped
...
- **`--diff-filter=ACMR` includes renames**, so the repo's own archival workflow —
  relocating a resolved proposal into `docs/proposals/archive/` — is itself a
...
- **Corrected on review — the 23 violations are not one class.** 22 are
  ...
  `docs/proposals/active/surface-lease-plane-v0.md`, which carries register hits too. Rule 5
