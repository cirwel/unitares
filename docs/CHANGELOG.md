...
- **impersonation guard, teaching:** three server surfaces taught calls that `inject_identity`'s impersonation guard refuses for a bound session: the observe router's example `observe(action='agent', agent_id=...)`, the `observe_agent` migration note, and the guard's own recovery text, which promised `knowledge(action='search', agent_id=X)` was unrestricted. The first two now teach `target_agent_id`, which `observe` has taken since 2.6.1, when this same "Session mismatch" was fixed by that rename. The recovery text and `read_path` now name `observe(action='agent', target_agent_id=...)`, say that a knowledge search without `agent_id` covers every agent's entries, and warn that removing `agent_id` from an observe call makes it a self-observe. The guard itself is unchanged. The 2026-09-13 council on identity posture decided against name exemptions: thesis, antithesis and request enforce ownership in their handlers, but synthesis does not, so for a bound caller this guard is the only barrier to submitting a synthesis as another participant. `tests/test_identity_guard_teaching.py` pins these points:
  ...
  - aliases of onboard/identity do not take middleware PATH 0, since canonical keying would let `start_session(agent_uuid=X)` bind X in log mode;
