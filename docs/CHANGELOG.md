...
- **outcome persistence:** prediction-bound `outcome_event` submissions are exactly-once per `(agent_id, prediction_id)`. The database claim and canonical outcome insert commit atomically; identical retries return the canonical existing outcome, while conflicting reuse returns `PREDICTION_REUSE_CONFLICT` with its ID. The unpartitioned claim ledger expires no later than the canonical outcome retention window. Calibration delivery remains a post-commit in-process side effect: an ambiguous commit can persist the outcome while delivering calibration zero times, tracked separately in #2247 rather than hidden by an outbox in this change. (#2246)
