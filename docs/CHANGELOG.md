...
### Changed
- **tool schemas:** `complexity` and `confidence` on `process_agent_update`, `simulate_update` and `sync_state` now advertise what the server accepts. The 0-1 bound had been emitted as Pydantic `ge`/`le`, which are not JSON Schema keywords, so every client validator ignored it and 99 was advertised as legal; it is now `minimum`/`maximum`. The string branch had allowed any string, so `"x"` looked legal too; it is now a regex of numeric strings in [0, 1]. `sync_state`'s `complexity` also lists the named levels its normalizer maps to numbers. The string branch was kept rather than dropped because the MCP transport builds its argument model from the advertised schema: without one, `sync_state(complexity="medium")` is refused before the normalizer runs, and a bad value on the canonical tools gets a generic parse error instead of the message naming the accepted forms. What the server accepts, refuses, and says when refusing is unchanged for every input on every transport. Three schema digests move; the negotiated interface release advances to 1.9.0 (1.8.0 is taken by a concurrent change). New guards: an invariant test that every boundary value an advertised schema calls legal passes both the transport and the handler path dispatch runs, alias normalizers included; a ban on Pydantic-only constraint keys in any advertised schema; and a `format`/`pattern` policy under which a construct the test cannot model fails loudly instead of being skipped. (#2181)
