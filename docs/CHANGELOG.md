...
- **dialectic synthesis:** a synthesis now requires a caller bound by identity resolution and submitting as the identity it names (`dialectic/auth.py` `require_bound_caller`, `caller_is_bound_as`). The participant allow-list checks which identity is named, not who is calling. `tests/test_dialectic_synthesis_bound_caller.py`.
