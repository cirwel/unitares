...
- **cirs_protocol:** `resonance_alert` and `stability_restored` are reachable through the schema. `_CIRS_DISPATCHERS` routes seven protocols, and the tool's own recovery text and docstring list all seven, but the `protocol` Literal admitted five: the two oscillation protocols were refused by `validate_params` on every transport ("Invalid value for 'protocol'"), because the Pydantic migration of 2026-02-28 wrote the Literal without the handlers that had landed on 2026-02-19. The Literal now matches the dispatch table, held there by a test, and the nine parameters the two handlers read (`oi`, `phase`, `tau_current`, `beta_current`, `flips`, `duration_updates`, `max_age_minutes`, `tau_settled`, `beta_settled`) are declared with the handlers' defaults and classified in `ACTION_FIELDS`. This is a reachability change, two protocols becoming callable that were not, so it is its own commit in the PR. Not changed: `resonance_alert` query filters on `agent_id`, and `cirs_protocol` is not in `inject_identity`'s `browsable_data_tools`, so a bound caller who omits `agent_id` has their own injected and one who names a peer is refused; through dispatch the query returns only the caller's own signals. (#2183)
...
- **tool_usage telemetry:** `cirs_protocol` declares its sub-action vocabulary (`emit`, `query`, `compute`, `set`, `get`, `list`, `initiate`, `respond`, `status`), so `audit.tool_usage.payload` records which action each call used. The tool takes `protocol` and `action`, but its decorator declared no `known_actions`, and `build_tool_usage_payload` treats a registered tool with no vocabulary as single-purpose, so every call landed as one undifferentiated row: state 3 of the measurement-authority taxonomy, "not recorded" (tool-surface audit F4, #2167). The set is the union of what the seven protocol handlers route on, and a test holds it to the handler bodies rather than to the schema prose; it is the telemetry clamp only, with no gate and no change to dispatch, and no `default_action` because no protocol defaults one. The schema's `action` description, which listed seven of the nine, now says which protocol takes which, and `describe_tool(tool_name="cirs_protocol", action=...)` answers per action. Descriptions live inside the hashed schema, so `cirs_protocol`'s `input_schema_sha256` and the catalog's `surface_sha256` move, the negotiated interface release advances to 1.7.0, and the contract artifact is regenerated; no parameter name, type, default or requiredness changes. (#2172)
- **cirs_protocol:** the wire schema declared three of the thirty-six parameters its protocol handlers read; twenty-four of the missing ones (`since_hours`, `filter_agent_id`, `filter_severity` and `context_ref` for `void_alert`; `include_trajectory`, `agent_ids`, `regime`, `max_risk` and `min_coherence` for `state_announce`; `source_agent_id` and `min_similarity` for `coherence_report`; `trust_default`, `trust_overrides`, `void_response_policy`, `max_delegation_complexity` and `accept_coherence_threshold` for `boundary_contract`; `action_type`, `payload`, `action_id`, `accept`, `response_data`, `as_initiator`, `as_target` and `status_filter` for `governance_action`) were dropped by FastMCP's argument model before dispatch. Over `/mcp/` a `void_alert` query therefore ignored its window and filters, a `governance_action` respond or status had no `action_id` to act on, and a `boundary_contract` set stored the defaults whatever the caller sent; REST and in-process callers were unaffected, because the middleware merges undeclared keys back (the `observe` / `describe_tool` class, interface release 1.2.0). Every one is now declared, with the handler's own default where it has one, and classified in `ACTION_FIELDS` under the action that reads it, so `describe_tool(tool_name="cirs_protocol", action=...)` shows them. Verifying it found a second defect of the same class, on every transport: the middleware hands a handler every declared field, `None` included, and the three query handlers did `int(arguments.get("limit", 50))` on the `None` the already-declared `limit` produced, so `cirs_protocol(protocol="void_alert" | "state_announce" | "coherence_report", action="query")` failed with a `TypeError` through dispatch while the handler tests, which call the handlers directly, passed. `limit` now advertises and delivers the handlers' default of 50, and `severity`, `filter_severity` and `action_type` are read None-tolerantly, so their structured refusals fire instead of an `AttributeError`. `tests/test_cirs_protocol_wire_params.py` derives the keys each protocol handler reads from its source and holds the schema, its defaults and its `ACTION_FIELDS` to them, and exercises the registered FastMCP tool end to end; the inventory is scoped to each handler's own call graph, because `resonance.py` holds two protocol handlers whose parameters are not interchangeable. The remaining nine missing parameters arrive with the two protocols in the Changed entry above. Additive on the wire, so the negotiated interface release advances to 1.8.0 and the contract artifact is regenerated. (#2183)
