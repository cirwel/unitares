...
Anchors are function names, not line numbers: the line anchors this table first
carried had gone stale by 2026-09-25 and were replaced that day, re-read
against `src/behavioral_sensor.py` and `src/governance_monitor.py`. Weights are
as coded; the post-blends are applied in `compute_behavioral_sensor_components`.
...
direct incremental-information path is **PATH BOUND** — `_compute_V` reads only
`E_history` and `I_history`, so it cannot add exogenous information beyond those
inputs. This is a source/path result, not a negative experiment and not a finding
about the policy-layer outcome residual. This does not
...
| `REFUTED` | a pre-registered, adequately powered test of the stated estimand failed its predeclared standard; no current EISV outcome claim has this status |
...
| `IMPLEMENTATION MISMATCH` | direct source, configuration, or algebra contradicts an exact description of the deployed mechanism; this is an engineering finding, not a negative experiment |
| `FORMULA MISMATCH` | deployed algebra differs from the named interpretation; the formula is characterized, not scientifically falsified |
| `PROVENANCE MISMATCH` | a label or signal source differs from its stated provenance; downstream outcome meaning must be re-identified |
| `DOCUMENTATION MISMATCH` | documentation and deployed behavior disagree; this is a correction obligation, not experimental evidence |
| `PATH BOUND` | source or algebra excludes a named direct path under stated configuration; recursive, counterfactual, and broader capability claims remain open |
| `EVENT RECONCILED` | audit records distinguish what was computed, recorded, delivered, or suppressed for a named event |
| `DEPLOYMENT SNAPSHOT` | a dated census describes this deployment; rows and events are not independent trials and the result does not generalize by itself |
...
`REFUTED BY CONSTRUCTION` is retired as an active status. It collapsed source
review, path analysis, and scientific falsification into one negative-sounding
bucket. Historical uses map to `IMPLEMENTATION MISMATCH` or `PATH BOUND` and
remain searchable as provenance; neither status is evidence against UNITARES as
a capability or against an outcome-level hypothesis.

The initial 2026-08-22 correction and its five reclassified claims are recorded
in
[`falsification-inference-containment-2026-08-22.md`](falsification-inference-containment-2026-08-22.md).
The systemic follow-up audits all 45 ledger rows, the outcome-test design, and
the recurring read protocol in
...
**2. Individuality of the I dimension** → **IMPLEMENTATION MISMATCH in
...
**3. The per-agent reference does useful work** → **BENCHMARK FAIL for the
...
**6. The six-component model measures six things** → **IMPLEMENTATION MISMATCH.**
...
**7. Π (preferences) contributes** → **PATH BOUND.** The producer
...
**8. ρ (recovery) contributes** → **PATH BOUND.** ρ is
...
**9. α (attractor) discriminates** → **PATH BOUND** for the
...
**10. `lineage_similarity` is telemetry-only** → **IMPLEMENTATION MISMATCH.** It is on the hot
...
**11. Drift alerting has discriminated anything** → **DEPLOYMENT SNAPSHOT: NO
...
*Amendment (2026-08-06): the anchors moved; the descriptive mechanism finding
stands, but it does not establish a refutation of individuality.*
...
**12. Lumen's Pi-side lineage is a six-component signature** → **IMPLEMENTATION
MISMATCH.** `anima-mcp` runs the same maths (same six weights, same
...
**13. "Behavioral EISV is observation-first — no ODE"** → **IMPLEMENTATION
MISMATCH.** The banner at `src/governance_monitor.py:1157` says so; the
...
**14. "Post-warmup enforcement can escalate on check-in content"** → **PATH
BOUND.** For a baselined agent under default flags, the only risk components
...
rank-immune additive term"** → **IMPLEMENTATION MISMATCH.** The row-2 agent-blind
...
**16. "The coherence gates can fire on the deployed signal"** → **PATH BOUND**
for the deployed V range (roadmap §4d/§9 asserted deadness; this row
...
dynamics"** → **IMPLEMENTATION MISMATCH**, on both counts.
...
**18. "AdaptiveGovernor owns threshold management"** → **PATH BOUND**
...
**19. "The I residual is scored against the agent's own sigma"** → **DEPLOYMENT
SNAPSHOT: SHARED FLOOR DOMINATES I.** `z_score` takes max(std, 0.05) (`agent_behavioral_baseline.py:52-66`;
...
**20. Row-2 open question (a): "are E and S also fleet-common?"** → **DEPLOYMENT
SNAPSHOT: E MAJORITY-DEGENERATE; S VARIES** (a different mechanism than row 2).
No agent-blind API feeds E or S — every input is
...
**21. "ΔEta's Consistency pillar is measured"** → **PATH BOUND.**
...
**22. "auto_ground_truth grades against exogenous signals"** → **PROVENANCE
MISMATCH.** The gate (`auto_ground_truth.py:207-240`) checks only that
...
**23. "Named residents run on measured per-class S setpoints"** → **IMPLEMENTATION
MISMATCH: DEPLOYMENT CONFIGURATION.** Residents classify as their own label (`class_indicator.py:102-104`);
...
bound and hold it there." — **PATH BOUND for the direct same-check-in
...
### Fitted and learned estimators — which bucket they sit in (2026-09-25)
...
**Fitted to outcome labels ⇒ efficacy, label-gated.** A fitted or
outcome-learned estimator — blend weights fitted against outcomes, or a learned
readout from EISV/prior state to an outcome — is **efficacy** work in the sense
of `docs/ROADMAP.md` (the "This item gates **efficacy** claims" paragraph under
"Now"). It is gated like any other efficacy claim, and the outcome-grounding
question it depends on belongs to the registered 2026-12-01 read. That read
tests only its pinned candidates; it does not adjudicate a new fitted
estimator. It is not instrument-frame work.
The ROADMAP's instrument-frame "calibration" is the confidence-vs-correctness
regime this document assigns to I in the sensory class split above. It does not
mean fitting the instrument to the outcome it is later scored against. Fitting
one before the read would also put an outcome-discrimination result in front of
the operator in the pre-read window. That was one of the reasons #2410 was
parked on 2026-09-24. It may be revisited after the 12-01 read reports, and only
...
baselines in place of the expanding Welford ones, and system identification of
the dynamics against an agent's own series need no outcome labels. They sit in
the instrument-frame bucket and may proceed offline, as analysis and design
notes. **Any live-path change from that work waits for the 2026-12-01 report:**
baselines, weights, verdict inputs, or thresholds. The reason is the one behind
the `decision_e` park above. Changing a predictor before the read contaminates
the registered series.

**Supply is thin (inference).** The 2026-09-23 relational calibration
instrument-supply read
(`docs/evaluations/relational-calibration-instrument-supply/result-v1.md`)
returned `instrument_supply_not_ready`: `strict_supply = 0` against a frozen
threshold of 200; its funnel fell from 38 `behavioral_mature` identities to 0
at `temporal_established`. That
read measured the supply for one registered instrument. Taking it to mean that
