...
- **2026-09-26 start_session envelope.** Canonical `onboard()` still returns the rich `thread_context` unchanged. The `start_session` alias's routine envelope does not repeat it: a fresh mint at `sibling_locus` (no declared, provisional or rejected lineage, no spawn reason other than `new_session`, and a live earlier node to name) carries `state_summary.episode_fork_kind` beside `predecessor_uuid`, and `next_action` says co-location does not establish lineage, which is the honest message's substance. `identity_lineage` and any unknown kind keep the whole onboard record under `raw_governance`, with `episode_fork_kind` lifted into `state_summary` the same way; a missing kind keeps the record and lifts nothing.

---

## Implementation status of dependencies (verified 2026-05-05)

Three runtime conditions verified against the running system review pass 2026-05-02:
...
- **`sibling_locus`, freshly minted UUID** (onboard only, added 2026-09-23): classification also routes a `force_new` mint that lands on a thread earlier, unrelated process-instances have occupied (a shared IP:UA fingerprint) to `sibling_locus`. The text above is false there, since a UUID *was* minted, so onboard passes `minted_fresh` and says: "You are a distinct subject - a fresh UUID on a thread that earlier process-instances have also occupied. Sharing a thread declares no lineage: they are not your predecessors. Memory access … yours to demonstrate, not asserted." The thin `process_agent_update` variant cannot tell the two cases apart from `ctx.meta`, so it passes `minted_fresh=None` and says only what holds for both (added 2026-09-24, #2375): "You are a distinct subject - a process-instance on a thread that other process-instances have also occupied. Sharing a thread declares no lineage: only a declared, distinct parent_agent_id does. Memory access … yours to demonstrate, not asserted."
...
| Existing `predecessor` field on onboard responses | Preserved inside the rich `thread_context`. R6 v2.1 adds sibling keys but does not move `predecessor`. The two `predecessor` shapes that exist (top-level vs inside thread_context) per live verification F10 are pre-existing and remain separate. | None |
