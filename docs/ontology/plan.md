...
## Status board (snapshot 2026-05-06)
...
OPEN — actionable
  ...
  R5       memory-deepening tooling — v0.1 KG cite-and-extend spec landed 2026-05-05; shadow scorer + synthetic tests + read-only diagnostic/batch sampler landed 2026-05-06, kept off the R2 hot path. Repeat live sample on 2026-05-06: 4 provisional pairs (3 insufficient parent memory, 1 absent) + 3 confirmed pairs (3 insufficient parent memory); `shadow_assessment` now explicitly returns `defer_runtime_gate` / `parent_memory_corpus_sparse`; durable audit table deferred until non-sparse integration evidence exists.
  R6       harness-substrate plurality dogfood track — v2 process_agent_update response-shape decision shipped 2026-05-02; `build_fork_context` prerequisite fixed by #284; `force_new` thread-position prerequisite implemented 2026-05-05; v2.1 onboard rich `thread_context` field shape shipped 2026-05-05; H1/H3 payload/assessment helper shipped 2026-05-06. Remaining: run controlled H1/H3 dogfood + broader candidate envelope evidence.
...
DEFERRED
  R2-Ph2   Trigger = ≥4 weeks Phase 1 telemetry plus design-doc gate (≥50 promoted pairs + ≥10 demoted pairs + ≥1 rejected-cross-role event observed); `scripts/diagnostics/r2_phase1_telemetry.py` live run on 2026-05-06 reports defer (1.97d, 3/50 confirmed, 0/10 demoted, 2/1 cross-role; remaining deficits now emitted under `failed_checks`); scope = trust-tier S6, KG provenance S7, R3 baselines, dashboard interpretation
  S14      Option C feasibility re-eval; trigger = ≥4w S1-a telemetry + ≥10 R4-passing agents
```

**Critical path now (2026-05-07 refresh):** S21 is closed end-to-end (S21-a 2026-04-28, S21-b via #231 + #245), S1-a/S1-b are shipped, S8a Phase 2 is shipped (#252), **S8b backfill is complete as of 2026-05-05**, **R2 Phase 1 shipped in #357 on 2026-05-05** (migration 036 + storage helpers, FSM/audit, cross-role pre-check, sweeper, process-update trigger), and **R2 Phase 1 telemetry now has a read-only diagnostic** (`scripts/diagnostics/r2_phase1_telemetry.py`) whose live run defers Phase 2 (1.97 days observed, 3/50 confirmed pairs, 0/10 demoted pairs, 2/1 cross-role rejections) and emits `failed_checks` with the remaining deficits. **R5 v0.1 KG cite-and-extend spec landed 2026-05-05 and its read-only shadow scorer/tests/diagnostic/batch sampler landed 2026-05-06**, with repeat live sampling still showing sparse parent KG memory rather than integration evidence; the batch output now says that directly via `shadow_assessment.decision=defer_runtime_gate`. **S7 lineage-chain provenance v0.1 is closed as of 2026-05-06** with schema decision, persistence round-trip, authoritative chain builder, aggregation helper, S22 write-context merge, and an index-readiness/no-migration gate, **R6 episode-fork response-shape decision v2 is shipped 2026-05-02**, the `build_fork_context` call-site prerequisite is resolved by PR #284 (`eedf5203`), the `force_new` thread-position prerequisite is implemented as of 2026-05-05, **R6 v2.1 onboard rich `thread_context` field shape is shipped 2026-05-05**, and the H1/H3 dogfood helper is available at `scripts/diagnostics/r6_dogfood.py`. R1 has shipped through §4.3 public KG emission (#306/#309/#314/#320/#321/#324), with #341 deferring the R3 baseline consumer until a real reader exists, and an operator-facing 2026-05-05 maintenance sweep now covers provisional promotion, unsupported-lineage demotion via explicit `apply_orphans`, and public KG TTL archival; the first live sweep confirmed 1 of 3 provisional rows. **S22 is closed as of 2026-05-06**: write-context provenance, identity-response distinction, durable state-json persistence, the H5 comparable-entry diagnostic, missing-entry payload templates, and a live complete three-harness H5 key (`s22-h5-2026-05-06`) across Hermes, Claude Code, and Codex CLI are all present; underlying model noted as GPT-5.5. **Low-friction check-in support shipped 2026-05-07** via `scripts/dev/with_checkin.py`, giving agents an opt-in way to wrap tests, diagnostics, commits, pushes, or arbitrary commands with `recent_tool_results` and S22 context without mandatory hooks. Remaining near-term work: (a) let R2 Phase 1 telemetry mature before opening Phase 2 consumers, (b) repeat R5 shadow sampling after the KG has more response-linked parent memory and keep durable audit history deferred while `shadow_assessment` stays `defer_runtime_gate`, (c) run R6 H1/H3 controlled Hermes dogfood using the new helper, then judge broader candidate-envelope evidence, (d) use the check-in wrapper for comparable pair-data tasks by setting `UNITARES_COMPARISON_KEY` / `UNITARES_TASK_LABEL`, and (e) keep the `gh pr list` preflight discipline from the 2026-04-29 migration-drift collision before any new identity ontology or migration-slot work.
