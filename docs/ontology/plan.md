...
**Status:** Active private/internal planning archive. Public `CIRWEL/unitares` master removed internal ontology/RFC/handoff drafts in `27642d11`; do not reintroduce this file to the public repo without an explicit operator decision.  
...
Numerically ordered, with current state. Note: **S18 was never assigned** — no gap to fill. The 2026-06-11 ledger triage (DEFERRED rows) is folded in below; full triage table at `docs/ontology/ledger-triage-2026-06-11.md`.
...
RESOLVED (shipped or accepted)
  ...
  S7       ✅ 2026-05-06  KG provenance v0.1 closed — schema decision, persistence round-trip, authoritative chain builder, aggregation helper, S22 context merge, and index-readiness/no-migration gate shipped
  ...
  S20      ✅ 2026-05-06  client cache scope narrowing closed — slot-scoped helper/command contract, direct-writer parity, legacy-token rejection/strip, inventory flags, and operator flat-cache cleanup runbook
  ...
  S22      ✅ 2026-05-06  harness context provenance closed — process-update/KG write context, identity-response distinction, H5 comparable-entry diagnostic, missing-entry payload templates, and live complete H5 key `s22-h5-2026-05-06` across Hermes, Claude Code, and Codex CLI; underlying model noted as GPT-5.5
  ...
  R2-Ph1   ✅ 2026-05-05  #357 shipped honest-memory Phase 1: migration 036 + storage helpers, FSM/audit, cross-role pre-check + response fields, sweeper, and process-update trigger; telemetry diagnostic now reports explicit failed-check deficits; opt-in `scripts/dev/with_checkin.py` wrapper shipped 2026-05-07 to increase meaningful command/test/diagnostic/commit/push check-ins with outcome evidence
  S10      ✅ 2026-05-19  per-class fleet calibration aggregation shipped via #471 — `compute_metrics_by_class()` with `{bootstrapped, by_class}` envelope (descriptive stats only at class scope per the e-process council finding), write-site `class_tag` resolution at outcome_events.py, rebucket hook on `class_promotion_sweeper_task`, dashboard card + modal pivot. Plan memo at `docs/ontology/s10-fleet-aggregation-plan.md`.
  S1-c     ✅ 2026-05-23  post-grace cross-process-instance token reject shipped after #422/#491 telemetry re-run. Server now rejects token-only `onboard`, token-only `identity`, and token-only `bind_session` with `status=continuity_token_resume_rejected`; same-live-process PATH 0 ownership proof and explicit `client_session_id` binding remain accepted.
  S2/S3    ✅ 2026-05-23  managed auto-resume / cross-channel token carry-forward cleanup shipped via unitares-governance-plugin#40 and unitares#513. Routine check-ins and generic SDK/CLI calls no longer auto-inject `continuity_token`; resident/Watcher PATH 0 proof is now explicit.

OPEN — actionable
  R1       remaining implementation — operator-facing promotion + public KG 30-day archival sweeps shipped 2026-05-05 (`src/identity/r1_maintenance.py`, `scripts/migration/r1_lineage_maintenance.py`); first live sweep confirmed 1 of 3 provisional rows, left 2 inconclusive, and found 0 stale public KG score nodes. R1 v3.4 added explicit unsupported-lineage demotion via `apply_orphans`; remaining work is consumer wiring once a real R3/baseline reader exists (#341 remains the defer marker). 2026-05-28: reader bug at `src/db/mixins/state.py` (E/S channel mismap and dead `stability_index` read since `20684dd1`, 2026-03-26) fixed via #530 (`73e1f3cb`); empirical replay flipped 0/268 plausible verdicts (min post-fix 0.8686, +0.169 above threshold), so no threshold retuning required. Historical `audit.r1_score_audit.components` rows between 2026-04-01 and 2026-05-28 11:17Z carry the buggy E/S semantics — discontinuity to account for in any cross-date analysis; live ablation noted corpus sparsity (3 unique parent→successor pairs across 268 EISV-complete rows in 30d) which reinforces the v7 corpus-maturity blocker.
  R5       memory-deepening tooling — v0.1 KG cite-and-extend spec + shadow scorer/tests/diagnostic/batch sampler landed 2026-05-05/06 and remain intentionally off the R2 hot path. Current stance: keep sampling, but do not create durable audit tables or runtime gates while `shadow_assessment` is still `defer_runtime_gate` / `parent_memory_corpus_sparse`.
  R6       harness-substrate plurality — response-shape v2/v2.1 and durable fork-discriminator plumbing are shipped. H1/H3 are no longer "run next": live read-only diagnostics on 2026-05-21 and 2026-05-23 re-verified explicit keys `r6-h1-2026-05-08` and `r6-h3-2026-05-08` as complete. Promotion-scope question answered conservatively on 2026-05-23: `s22_candidate_envelope_coverage.py --since 2026-05-08T00:00:00Z` shows post-fix candidate envelope fields remain unearned; only sparse `locus`/`episode_id` traces appear. Keep S22 core write-context + fork discriminators; do not promote full candidate-envelope fields until H7/H8/gateway/Discord/cron/Dispatch dogfood earns them. Historical H1/H3 rows predate fork-discriminator persistence/backfill, so `episode_fork_kind` / `identity_lineage_fork` remain 0/2 on those explicit keys by expected no-backfill semantics.
BLOCKED — by upstream work
  Q1, Q2   blocked on R1 / R2
  S9       blocked on R1 for broader PATH 1/2 re-scope; bind_session TTL coupling now pinned by handler regression (2026-05-06)
  S12      demoted to phenomenological in v7 §3; unblock requires asymmetric-info channel maturity and a fresh channel-geometry pass, not date expiry alone

DEFERRED
  R2-Ph2   Trigger = ≥4 weeks Phase 1 telemetry plus design-doc gate (≥50 promoted pairs + ≥10 demoted pairs + ≥1 rejected-cross-role event observed); current plan posture remains defer until telemetry is genuinely non-sparse and downstream consumers can distinguish forward-only chain credit from retrospective identity inheritance.
  S14      Option C feasibility re-eval; trigger = ≥4w S1-a telemetry + ≥10 R4-passing agents
```

**Triage 2026-06-11:** Time-based triggers for both DEFERRED rows have elapsed — R2-Ph2 (≥28 telemetry days, 37 elapsed since the 2026-05-05 Phase 1 ship) and S14 (≥4w S1-a telemetry, 43 days since 2026-04-29). Neither is unblockable: R2-Ph2 still needs its live confirmed/demoted/cross-role counts (R1's 2026-05-28 sparsity note — 3 unique parent→successor pairs / 268 EISV-complete rows — implies confirmed_pairs far below the 50 gate), and S14's ≥10 R4-passing (substrate-earned) agents is implausible today with only a handful of substrate-anchored residents. No BLOCKED row's upstream (R1 consumer wiring, R2 Phase 2, asymmetric-info channel maturity) has closed. Full table + live-verification commands: `docs/ontology/ledger-triage-2026-06-11.md`.

**Critical path now (May 23, 2026 refresh):** Public `origin/master` has intentionally removed internal ontology/RFC/handoff drafts (`27642d11`), so this archive is now the private planning surface rather than a public-repo doc. The stale branch/PR/worktree backlog that obscured this surface is cleared; hygiene reports no holds, Watcher has no open findings, and the only remaining local branch is an archive stash preserving sequential-calibration backup data. S1-c is closed after the Hermes telemetry blocker cleared, and the immediate S2/S3 managed-surface cleanup is also closed. S10/S22 are closed, R6 H1/H3 dogfood is complete on explicit historical keys, and the 2026-05-23 promotion-scope refresh says not to promote the broader candidate envelope yet. Near-term R6/S22 work should generate targeted H7/H8/gateway/Discord/cron/Dispatch evidence, not widen public schemas or declare sparse candidate fields earned.

**Operator wedge:** (a) run coverage diagnostics before every R6/S22 promotion claim, using `--since` when evaluating post-fix rows; (b) treat date-default R6 diagnostic emptiness as key rollover unless explicit intended keys fail; (c) keep R2 Phase 2 and R5 runtime/audit gates deferred while evidence remains sparse; (d) watch the FastMCP `process_agent_update` argument boundary when Hermes/native MCP rows are missing S22 fields — patch server-side passthrough rather than widening public schemas or replaying through REST as a permanent fix.

## Ledger

Every item from `identity.md` that requires work, what "resolved" means for it, and what it depends on.

> **History of record.** The cells below preserve each row's full decision trail — including options that were withdrawn and claims later retracted (e.g. S1 Option B, S11-a's sibling-audit). That trail is deliberately kept rather than rewritten; for the one-line live state of any row, use the Status board above.
...
| Q3 | Paper positioning — v7 thesis or implementation detail? | Nothing. Pure re-read. | **Resolved 2026-04-21** — recommendation (v7 animating thesis) at `docs/ontology/paper-positioning.md` accepted by the operator. Downstream work: v7 outline draft in `unitares-paper-v6` repo, timing TBD. |
...
|---|---|---|---|---|
| S1 | `continuity_token` as resume-credential | **Deprecate with grace period** — **plan doc shipped 2026-04-24** at `docs/ontology/s1-continuity-token-retirement.md` (reviewed by dialectic-knowledge-architect + code-reviewer council). Surfaces Part-C coupling (token is load-bearing for PATH 0 anti-hijack, 2026-04-18) that makes naive retirement unsafe. Options A (narrow TTL — recommended pragmatic first step, honestly labeled "performative, narrowed"), A′ (narrow + PID/nonce bind — ontology-clean follow-on), C (substrate-only). Option B (wait for R1) **withdrawn 2026-04-24** after R1 v3 scope clarification — R1 is plausibility scoring, not authentication, and does not replace the token's anti-hijack role. Forward-compat `ownership_proof_version` field designed to make A → A′ non-breaking. **Operator decisions accepted 2026-04-25** (path A → A′; TTL 1h; grace = one release cycle warning-only; `bind_session` let-propagate; Chronicler re-onboard-on-wake acceptable; field rename deferred to S1-d post-grace; ontology label "performative, narrowed"). Implementation sequencing (S1-a → S1-b → S1-c → S1-d → A′) stands per §9. **S1-a shipped 2026-04-29** — full chain merged in one PR after S21-a/b unblocked the wiring path: TTL shrink + opv injection (already in master pre-2026-04-29), shared `_emit_continuity_token_deprecation` helper landed, wired across `handle_onboard_v2` (refactored from inline), `handle_identity_adapter`, `handle_bind_session`; HTTP REST inherits transitively through `execute_http_tool` dispatch (item 3 verified obsolete). `_CLOCK_SKEW_TOLERANCE = 30s` added to `resolve_continuity_token` per §7.2 (bounded below 60s minimum TTL). 6 new regression tests (clock-skew window, expiry-mid-call, concurrent-possessors-with-expired, Chronicler-shape >1h-old token, plus 2 wiring-presence tests). Operator pairs ship with secret rotation per §11.7. **S1-c shipped 2026-05-23** after #422/#491 telemetry re-run: token-only `onboard`, token-only `identity`, and token-only `bind_session` reject with `status=continuity_token_resume_rejected`; same-live-process PATH 0 ownership proof and explicit `client_session_id` binding are preserved. **Cross-link (#807/#810, resolved 2026-06-17, doc #816):** S1-c's token-narrowing is the reason the tier-honesty debt has no token-based remedy — a copyable cross-process `continuity_token` would only relocate the over-claim S1-c already closed. The tokenless `agent-{uuid}` prefix-echo's `strong`/`caller_proven` stamp is therefore a **known, bounded, by-construction over-claim** (named in `docs/ontology/identity.md` §"Transport-level continuity"), not a token regression. Resolution was **honest-by-disclosure** (no tier-code change); the only honest `strong` cross-process path is substrate attestation (S19 / Option C, see S14). The strict-write-gate enforcement question stays open in #807, gated to #425 stage-4. | None. Scope clear. | S1-a: TTL shrink + deprecation-warning + `ownership_proof_version` ✅ shipped 2026-04-29 to `unitares` master. S1-b: onboard-helper + CLI startup-path migration to `force_new + parent_agent_id` shipped to plugin + unitares. S1-c: cross-process-instance reject ✅ shipped 2026-05-23. S1-d (optional): field rename `continuity_token → ownership_proof`. A′: PID/nonce binding bumps `ownership_proof_version` to 2. |
| S2 | `.unitares/session.json` auto-resume | **Retired 2026-05-23** | S1-c | Auto-resume removed from Codex plugin + Claude Code harness hooks earlier; 2026-05-23 cleanup closed the remaining routine check-in / client carry-forward surfaces. `unitares-governance-plugin#40` stops passing `continuity_token` to `process_agent_update`; `unitares#513` stops SDK, sync SDK, CLI, and `with_checkin` generic tool calls from auto-injecting it. Fresh processes mint fresh identities with declared lineage; in-process work uses explicit `client_session_id`. |
| S3 | Cross-channel token acceptance | **Retired 2026-05-23 for managed surfaces** | S1-c | S1-c rejects token-only cross-process resume; the 2026-05-23 plugin/server cleanup removed generic cross-channel token carry-forward from managed check-in, SDK, CLI, and Watcher paths. Same-live-process PATH 0 ownership proof remains explicit (`identity(agent_uuid, continuity_token, resume=true)`) until S1-d / A′ decides whether to rename or bind the proof field. |
...
| S7 | KG provenance (`agent_id` stamping) | Audit + shift aggregation | R3 | **Schema decision v0.1 landed 2026-05-06** at `docs/ontology/s7-kg-provenance-lineage-schema.md`. Decision: use existing `knowledge.discoveries.provenance_chain` JSONB as immutable write-time lineage snapshot, keep `core.identities` as canonical current edge state, and distinguish `as_written` vs `current_valid` aggregation modes. **Persistence round-trip shipped 2026-05-06:** single-store handler passes `provenance_chain` into `DiscoveryNode`, PG storage inserts/conflict-updates it, and Postgres hydration returns it. **Authoritative snapshot builder shipped 2026-05-06:** `src/identity/provenance_chain.py` walks `core.identities`, records R2 lifecycle columns per link, and stops with explicit `chain_stop_reason` instead of fabricating missing ancestry. **Aggregation helper shipped 2026-05-06:** read-side helpers answer `as_written` vs `current_valid`, exclude provisional links by default, and claw back currently demoted/archived/reparented chains without flipping existing stats endpoints. **S22 context merge shipped 2026-05-06:** `src/provenance_context.py` records optional harness/model/transport/memory/tool/locus context under KG `provenance.s22_context` and process-update `agent_state.provenance_context`. **Indexing decision shipped 2026-05-06:** `src/identity/provenance_index_readiness.py` plus `scripts/diagnostics/s7_provenance_index_readiness.py` keep the default at no JSONB/generated-column migration until row volume and observed JSONB query pressure cross thresholds. S7 v0.1 is closed; reopen only when the diagnostic reports `candidate` or a concrete consumer needs different query semantics. |
...
| S9 | PATH 1/2 anti-hijack machinery | Re-scope or retire | R1 | Under R1, external verification replaces continuity-enforcement. PATH 1/2 flip to lineage-plausibility checks or retire. **`bind_session` TTL-coupling parked here 2026-04-25** — under S1-a's TTL shrink (already shipped at `_CONTINUITY_TTL = 3600`), `bind_session`'s acceptance window silently changed from 30d to 1h. Handler-level regression coverage shipped 2026-05-06 in `tests/test_identity_handlers.py::TestHandleBindSession::test_bind_session_rejects_expired_continuity_token_before_resolution`; broader PATH 1/2 re-scope or retirement still waits on R1. |
| S10 | Fleet calibration aggregation paths | **Resolved 2026-05-19** | R3, S7 | **Shipped 2026-05-19** as [unitares#471](https://github.com/CIRWEL/unitares/pull/471). Default fleet calibration view shifts to per-class as the primary axis, with global preserved as the all-bucket sum and UUID retained as drill-down. Three coupled changes: (1) `SequentialCalibrationTracker` gains `class_states` denormalized rollup, `compute_metrics_by_class()` returning `{bootstrapped, by_class}` envelope, `rebucket_from_agent_states(classifier)` primitive, `class_states_bootstrapped` flag honestly labeling the gap between pre-S10 state-file load and first sweeper rebucket, and `UNKNOWN_CLASS_BUCKET` fallback (`src/sequential_calibration.py`); (2) `outcome_events.py:332` resolves `class_tag = classify_agent(agent_metadata.get(agent_id))` at write time with `default` fallback on cache miss and `None` (→ UNKNOWN bucket) degradation on lookup exception; `check_calibration` MCP response carries `by_class` envelope; `class_promotion_sweeper_task` tails each 30-min cycle with a rebucket pass (isolated try-block so rebucket failure cannot starve promotion); (3) dashboard calibration card detail line leads with per-class chips, modal expands to per-class table with `bootstrapping…` banner during the gap. **Council pass on the pre-merge draft** (`dialectic-knowledge-architect` + `feature-dev:code-reviewer` + `live-verifier`) produced one math finding and three concrete asks, all folded in before merge: (a) class-scope envelopes omit `log_evidence` / `capped_alarm` / `last_alt_probability` / `last_e_value` unconditionally because summing per-agent `log_e_values` across different q-trajectories has no martingale interpretation — the module docstring already warns global × per-agent product is forbidden under H0, and class-states is the same forbidden product, so class scope keeps only descriptive statistics that survive aggregation; (b) rebucket classifier exceptions log to stderr (type + agent_id + message) and increment `classifier_errors` telemetry; (c) `class_states_bootstrapped` flag added so dashboards label the pre-S10 bootstrap window instead of misleadingly showing 7 unknown vs 5525 global samples; (d) live-verifier refuted the original plan's "caller has meta in scope" assumption at the write site — outcome_events.py:332 has only `agent_id`, so S10.2 wires the `agent_metadata.get(agent_id)` lookup there; verifier's "100% unknown" claim was wrong because class lives in `metadata.tags` JSONB array (218 ephemeral, 202 engaged_ephemeral, ~10 persistent variants, 1 embodied, 8 untagged out of 437 active per the 2026-05-19 cross-check). Plan memo at `docs/ontology/s10-fleet-aggregation-plan.md`. 187 tests green across the touched neighborhood. **Honest re-framing of the original row:** the row read "shift default from UUID to role," but the live default before S10 was *global*, not UUID — per-UUID partition was structurally wired but no reader consumed it. The shipped semantics: *add* per-class as the primary fleet view, preserve global as the all-bucket sum, retain UUID as drill-down. |
...
| S20 | Client cache scope narrowing — close helper allowance + umask gap on direct-writer (hook layer already shipped via PR #19) | **Resolved 2026-05-06** — plan doc at `docs/ontology/s20-cache-scope-narrowing.md`. Addresses cache *placement* (orthogonal to S2's semantic auto-resume retire and S1's token-format cuts). Honestly labeled **convention-level, advisory**: helpers stop teaching the shared-cache pattern, while earned defense remains S1-A′ + S19. | None. | **S20.0 answered 2026-04-25. S20.1a resolved 2026-04-26** via plugin#23. **S20.1b resolved 2026-04-27** via plugin#25 (`cmd_set` rejects slotless writes/token-bearing v2 payloads and exposes `cmd_list`). **S20.2 resolved 2026-05-01** via plugin#30. **S20.3 resolved 2026-05-01** via unitares#276 + plugin#29 (direct-writer mode 0600, no cache-persisted token). **S20.4 resolved 2026-05-06** via unitares-side `scripts/client/session_cache.py` slot enforcement/list inventory/token rejection plus Codex command docs. **S20.5 resolved 2026-05-06** via operator runbook cleanup guidance and non-secret inventory flags (`legacy_flat`, `has_legacy_token`, `schema_version`). **S20.6 resolved 2026-05-06 for unitares-side surfaces** via helper/direct-writer/command-doc tests; plugin hook regressions shipped with S20.1/S20.2. |
| S22 | Harness context provenance — preserve harness/model/transport/tool-surface context on governance writes and KG provenance | **Resolved 2026-05-06** | R6, S7 | Resolved when `process_agent_update` can record optional harness/model/transport/tool-surface metadata; KG writes expose that metadata in provenance; identity responses explicitly distinguish UUID, label, harness, and assurance; and Hermes, Claude Code, and Codex CLI each have one comparable recorded task entry. Draft framing at `docs/ontology/harness-substrate-plurality.md`. **Partial promotion 2026-05-02/05** via R6 v2/v2.1 (`docs/ontology/r6-episode-fork-response-shape.md`): two fields (`episode_fork_kind` enum + `identity_lineage_fork` boolean) promoted to `process_agent_update` thin `thread_context` and `onboard()` rich `thread_context`; remaining fields (harness/model/transport/tool-surface metadata, full candidate envelope) stayed gated on dogfood evidence per `harness-substrate-plurality.md` §"Design risks." **Write-context merge shipped 2026-05-06:** `process_agent_update` can persist optional S22 context and KG writes expose it under `provenance.s22_context`. **Identity-response distinction shipped 2026-05-06:** identity/onboard payloads now emit `identity_context.schema = "s22.identity_response.v1"` plus top-level `identity_assurance`, explicitly separating UUID registry anchor, public `agent_id`, social `display_name`, descriptive harness/model context, and session-resolution assurance. **H5 coverage gate shipped 2026-05-06:** `process_agent_update` now carries S22 context into durable `core.agent_state.state_json.provenance_context`, S22 write context accepts `comparison_key`/`task_label`/`task_outcome`, and `scripts/diagnostics/s22_h5_comparable_entries.py` reports whether one shared H5 key spans Hermes, Claude Code, and Codex CLI. **Live diagnostic compatibility fixed 2026-05-06:** H5 KG query no longer selects schema-optional `knowledge.discoveries.confidence`, asyncpg row normalization accepts live `Record` rows, and `--comparison-key` now filters both durable-state and KG queries so the shared H5 key cannot age out behind unrelated S22 writes. **Missing-entry payload support shipped 2026-05-06:** the diagnostic's `--show-missing-payloads` flag emits ready-to-send `process_agent_update` payload templates for missing required harnesses on the target key, so Hermes/Claude Code can record their own entries without field-shape ambiguity. **Teaching surface pinned 2026-05-06:** `describe_tool(process_agent_update)` now documents S22/H5 fields and tests assert both schema and description expose them. **H5 live gate closed 2026-05-06:** read-only diagnostic for `s22-h5-2026-05-06` now reports `decision=complete`, `reason=shared_comparison_key_covers_required_harnesses`, `complete_comparison_keys=["s22-h5-2026-05-06"]`, and comparable harnesses `["claude-code","codex-cli","hermes"]`; operator note records the underlying model as GPT-5.5. Hermes row was accepted through weak `ip_ua_fingerprint` identity assurance with confidence dampened, which is acceptable here because S22 records harness provenance, not strong identity proof. |

## Suggested sequencing

Historical tracks below can progress independently. As of May 21, 2026, Tracks A–C are mostly superseded by the status board; the live wedge is Track D.

### Track D: R6/S22 provenance promotion and archive hygiene
*Goal: keep ontology promotion evidence-backed while the public repo remains scrubbed of internal drafts.*

- **D1 (R6 H1/H3):** Treat `r6-h1-2026-05-08` and `r6-h3-2026-05-08` as the completed canonical Hermes dogfood keys; if date-default diagnostics return zero, check explicit keys before calling regression.
- **D2 (R6/S22 coverage):** Use `scripts/diagnostics/s22_candidate_envelope_coverage.py --comparison-key <key> --json` before any field-promotion claim. Historical H1/H3 keys still show fork-discriminator `0/2` because they predate durable persistence/backfill.
- **D3 (promotion boundary):** Promote core write-context and optional lifecycle fields only; defer full envelope fields (`affordance_state`, formal `locus`, `harness_id`, `episode_id`, `invocation_id`, `process_instance_id`, embedded `identity_assurance`, `client_session_id`, `label_at_write`) until H7/H8/gateway/Discord/cron/Dispatch evidence exists.
- **D4 (next evidence source):** Before proposing any candidate-envelope promotion, create a fresh comparison key for the specific harness surface being tested (H7/H8/gateway/Discord/cron/Dispatch), run `r6_dogfood.py --assess` and `s22_candidate_envelope_coverage.py --json`, and record the observed field ratios here. As of 2026-05-23, `r6_dogfood.py` emits and assesses H7 tool-surface pairs and H8 interactive/non-interactive transport pairs; use those targeted helpers before broader field promotion. Do not infer promotion readiness from date-default empty diagnostics or from historical H1/H3 rows.

**Exit criteria:** one evidence-backed promotion decision and a fresh diagnostic key if new fields are proposed.
...
- **C2 (S2, S3):** ~~Auto-resume + cross-channel removal.~~ **Done 2026-05-23** — S1-c rejected token-only cross-process resume; `unitares-governance-plugin#40` and `unitares#513` removed routine `continuity_token` auto-injection from managed check-in, SDK, CLI, Watcher, and resident handoff surfaces while preserving explicit PATH 0 ownership proof.
...
| **S10** (fleet aggregation paths) | ✅ Resolved 2026-05-19 via #471 — per-class as primary fleet view, global preserved, UUID as drill-down; class-scope envelope carries descriptive stats only per the e-process council finding |
...
### S20 — Agent Identity Credential (AIC): third-party-verifiable attestation primitive (prototype, 2026-06-24)

**What.** A self-contained prototype of an asymmetric, server-signed identity *attestation* — the verifiable-attestation building block S19 B-strict named ("server-verifiable or non-exportable attestation, not another copyable secret"). Code: `src/identity/agent_identity_credential.py` (+15 passing tests); design: `docs/proposals/archive/agent-identity-credential-aic-v0.md`.

**Stance (identity.md).** Descriptive, not Performative. The AIC reports accrued standing across the five-layer taxonomy (`uuid` / `structured_agent_id` / `role_family` / `substrate_class` / `trust_tier` + `observation_count` / `lineage_state`) and lets any third party verify the issuer's claim **offline** via a published JWKS public key — the asymmetric upgrade the symmetric-HMAC, issuer-verifiable-only `continuity_token` cannot provide. Passes "build nothing that appears more alive than it is": a dated, authority-free statement claims only what it can back.

**Why it does NOT reopen the S19 copyable-bearer vector.** An AIC is an attestation, not a bearer credential: no `sid`/session field, `resume_capable=false`, `authorizes=[]`, `is_resume_credential()` hard-False, and a deliberately disjoint `aic.v2.<…>` envelope (domain-separated signature) that will not cross-verify with a `v1.<…>` continuity_token. Copying it grants nothing — verifying it proves the *issuer attested these claims*, never *the presenter is this agent*.
...
**Answered 2026-09-25 (second question only):** no concrete external verifier exists, so AIC issuance stays unwired, `drr.v1` stays dormant, and multi-principal work is not scheduled until an outside party asks to verify a record. See [`federation-trust-decisions-2026-09-25.md`](../proposals/active/federation-trust-decisions-2026-09-25.md) (D3a). The first question stays open.
...
2026-09-02). Decision packet: `docs/proposals/archive/outcome-fixture-conflation-decision-packet-v0.md`
