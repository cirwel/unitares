...
OPEN — actionable
  ...
  R6       harness-substrate plurality — response-shape v2/v2.1 and durable fork-discriminator plumbing are shipped. H1/H3 are no longer "run next": live read-only diagnostics on 2026-05-21 and 2026-05-23 re-verified explicit keys `r6-h1-2026-05-08` and `r6-h3-2026-05-08` as complete. The 2026-05-23 coverage rerun again shows promoted core fields at 2/2 for both keys, optional memory/session/thread fields populated where meaningful, most candidate envelope fields unearned (`affordance_state`, `harness_id`, `episode_id`, `invocation_id`, `process_instance_id`, embedded `identity_assurance`, `label_at_write`, `agent_uuid`, `client_session_id` at 0/2; `locus` at 2/2 but still candidate), and historical fork-discriminator fields still 0/2. The next R6 question is promotion scope: keep S22 core write-context + optional lifecycle fields; do not promote full candidate-envelope fields until H7/H8/gateway/Discord/cron/Dispatch dogfood earns them. Historical H1/H3 rows predate fork-discriminator persistence/backfill, so `s22_candidate_envelope_coverage.py` still reports `episode_fork_kind` / `identity_lineage_fork` as 0/2 on those keys; treat that as no-backfill evidence, not a response-shape regression.
