...
| S9 | PATH 1/2 anti-hijack machinery | Re-scope or retire | R1 | Under R1, external verification replaces continuity-enforcement. PATH 1/2 flip to lineage-plausibility checks or retire. **`bind_session` TTL-coupling parked here 2026-04-25** — under S1-a's TTL shrink (already shipped at `_CONTINUITY_TTL = 3600`), `bind_session`'s acceptance window silently changed from 30d to 1h. **Regression-test pin shipped 2026-05-06** at `tests/test_s1_continuity_token_narrow.py::test_bind_session_handler_rejects_expired_continuity_token_s9` — a strict differential against the resolver-layer test (`test_bind_session_resolve_path_uses_1h_ttl`) that exercises `handle_bind_session` end-to-end and asserts (a) a fresh continuity_token binds successfully, (b) the same token time-traveled past `_CONTINUITY_TTL + _CLOCK_SKEW_TOLERANCE` is rejected at the early gate. Captures the failure mode the S1 council surfaced: a future swap of `resolve_continuity_token` for `extract_token_agent_uuid` (which intentionally ignores expiry per Part-C) would silently restore the pre-S1-a 30d-equivalent acceptance window on bind_session. The broader S9 question — re-scope or retire PATH 1/2 under R1 — remains blocked on R1. |
...
BLOCKED — by upstream work
  ...
  S9       blocked on R1 (bind_session TTL-coupling regression test shipped 2026-05-06 — see S9 row)
  S10      blocked on S7
...
**Critical path now (2026-05-05 refresh):** S21 is closed end-to-end (S21-a 2026-04-28, S21-b via #231 + #245), S1-a/S1-b are shipped, S8a Phase 2 is shipped (#252), **S8b backfill is complete as of 2026-05-05**, **R2 design v2 is shipped 2026-05-02** and its Phase 1 implementation row opened 2026-05-04, **R6 episode-fork response-shape decision v2 is shipped 2026-05-02**, the `build_fork_context` call-site prerequisite is resolved by PR #284 (`eedf5203`), the `force_new` thread-position prerequisite is implemented as of 2026-05-05, and **R6 v2.1 onboard rich `thread_context` field shape is shipped 2026-05-05**. R1 has shipped through §4.3 public KG emission (#306/#309/#314/#320/#321/#324), with #341 deferring the R3 baseline consumer until a real reader exists, and an operator-facing 2026-05-05 maintenance sweep now covers provisional promotion plus public KG TTL archival; the first live sweep confirmed 1 of 3 provisional rows. Remaining near-term work: (a) specify/implement destructive handling for unsupported lineage (`orphan_candidate`) if the operator wants automatic edge removal, (b) continue R2 Phase 1 and S22 broader harness/model/transport/tool-surface envelope after dogfood evidence, and (c) keep the `gh pr list` preflight discipline from the 2026-04-29 migration-drift collision before any new identity ontology or migration-slot work.
