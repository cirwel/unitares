...
**What.** A self-contained prototype of an asymmetric, server-signed identity *attestation* — the verifiable-attestation building block S19 B-strict named ("server-verifiable or non-exportable attestation, not another copyable secret"). Code: `src/identity/agent_identity_credential.py` (+15 passing tests); design: `docs/proposals/agent-identity-credential-aic-v0.md`.
...
`calibration_exclusion_reasons`, `calibration_excluded` stays the single calibration
gate, the instruments take `--fixture-rule` (default `registered`; `corrected` opt-in
