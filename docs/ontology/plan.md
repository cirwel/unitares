...
| R5 | Memory-deepening-reality tooling (axiom #14) | R2 (integration must be defined before deepening it) | **Design doc v0.1 landed 2026-05-05** at `docs/ontology/r5-memory-deepening-reality.md`. First channel selected: KG cite-and-extend, because existing KG `response_to` / provenance plumbing gives an orthogonal signal to R1 trajectory similarity without a new storage prerequisite. v0.1 defines `score_memory_integration(..., channel="kg_cite_extend")`, seeded verdicts (`integrated_candidate`, `weak_signal`, `absent`, `insufficient_parent_memory`, `inconclusive`), and a shadow-only implementation sequence. Runtime posture: R2 Phase 1 remains unchanged; R5 must not become a promotion conjunct until shadow telemetry establishes false-negative behavior. Deferred channels remain forced re-derivation, behavioral backtests, and self-knowledge reflection. |
...
OPEN — actionable
  ...
  R5       memory-deepening tooling — v0.1 KG cite-and-extend spec landed 2026-05-05; next action is a read-only scorer + synthetic tests, kept off the R2 hot path.
...
**Critical path now (2026-05-05 refresh):** S21 is closed end-to-end (S21-a 2026-04-28, S21-b via #231 + #245), S1-a/S1-b are shipped, S8a Phase 2 is shipped (#252), **S8b backfill is complete as of 2026-05-05**, **R2 Phase 1 shipped in #357 on 2026-05-05** (migration 036 + storage helpers, FSM/audit, cross-role pre-check, sweeper, process-update trigger), **R5 v0.1 KG cite-and-extend spec landed 2026-05-05**, **R6 episode-fork response-shape decision v2 is shipped 2026-05-02**, the `build_fork_context` call-site prerequisite is resolved by PR #284 (`eedf5203`), the `force_new` thread-position prerequisite is implemented as of 2026-05-05, and **R6 v2.1 onboard rich `thread_context` field shape is shipped 2026-05-05**. R1 has shipped through §4.3 public KG emission (#306/#309/#314/#320/#321/#324), with #341 deferring the R3 baseline consumer until a real reader exists, and an operator-facing 2026-05-05 maintenance sweep now covers provisional promotion, unsupported-lineage demotion via explicit `apply_orphans`, and public KG TTL archival; the first live sweep confirmed 1 of 3 provisional rows. Remaining near-term work: (a) collect R2 Phase 1 telemetry before opening Phase 2 consumers, (b) implement R5's read-only KG cite-and-extend scorer in shadow mode, (c) continue R6/S22 broader harness/model/transport/tool-surface envelope after dogfood evidence, and (d) keep the `gh pr list` preflight discipline from the 2026-04-29 migration-drift collision before any new identity ontology or migration-slot work.
