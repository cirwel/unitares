...
| S20 | Client cache scope narrowing — close helper allowance + umask gap on direct-writer (hook layer already shipped via PR #19) | **Open 2026-04-25** — plan doc shipped at `docs/ontology/s20-cache-scope-narrowing.md` (reviewed by `dialectic-knowledge-architect` + `feature-dev:code-reviewer` council; one-pass honesty re-review; **post-ship amendment 2026-04-25** correcting `hooks/session-start` claim — PR #19 `87affc9` already shipped slot-scoped read). Addresses cache *placement* (orthogonal to S2's *semantic* auto-resume retire and to S1's *token-format* cuts). The hook layer is already correct (slot-scoped read via Claude Code's SessionStart `session_id`, plus scan-newest fallback in plugin#30). Honestly labeled **convention-level, advisory** — bypassable by any caller writing JSON to the path; the earned defense is S1-A′ + S19. S20's job is to stop the system from *teaching* the shared-cache pattern, not to prevent a determined caller from re-creating it. | None — all sub-steps independent. | **S20.0 answered 2026-04-25. S20.1a resolved 2026-04-26** via plugin#23. **S20.1b resolved 2026-04-27** via plugin#25 (`cmd_set` rejects slotless writes, token-bearing v2 payloads, and exposes `cmd_list`). **S20.2 resolved 2026-05-01** via plugin#30. **S20.3 resolved 2026-05-01** via unitares#276 + plugin#29 (direct-writer mode 0600, no cache-persisted token). **S20.4 resolved 2026-05-06:** unitares-side `scripts/client/session_cache.py` now mirrors slot-scoped write enforcement (`--slot` required unless `--allow-shared`), rejects non-empty `continuity_token`, strips legacy tokens on merge, exposes `list` for scan-newest lineage, and Codex command docs now use slot inventory/slotted writes instead of flat `session.json`. **S20.6 resolved 2026-05-06 for unitares-side surfaces:** tests cover helper rejection, token rejection, list inventory, legacy-token strip, command-doc drift, and direct-writer mode 0600; plugin hook regressions shipped with S20.1/S20.2. **Remaining:** S20.5 operator-runbook cleanup guidance for pre-PR-19 flat caches. |
...
OPEN — actionable
  ...
  S20      PR1 merged (plugin#23); S20.1b merged (plugin#25) 2026-04-27; S20.3 merged (#276 + plugin#29) 2026-05-01; S20.2 merged (plugin#30) 2026-05-01 — second-pass caught a heredoc-pipe stdin bug the council missed; S20.4/S20.6 Codex-side helper+command contract landed 2026-05-06; remaining open item is S20.5 operator cleanup/runbook for pre-PR-19 flat caches
