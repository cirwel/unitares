...
RESOLVED (shipped or accepted)
  ...
  S6       ⚠️ 2026-04-23  Option B + Q2 reseed shipped; session-like recal awaits S8a Phase 2
  S8       ✅ 2026-04-21  re-scoped → S8a/S8b (thresholds remain)
  S8a      ✅ 2026-04-23  Phase 1 shipped (PR #121); Phase 2 trigger fires ~2026-04-30
  ...
  S21-b    ⚠️ 2026-04-30  items 1-4 ✅ #231 + 8 hardening commits; items 7 + 8 ✅ (audit-closed 2026-04-29); items 5 + 6 council-clean in PR #245 (awaiting merge): item 5 — middleware threads resolved identity through `arguments` so `handle_onboard_v2` and `_try_resume_by_session_key` consume it instead of re-resolving; item 6 — `require_registered_agent` consults middleware-threaded `core_agent_row_status` (sourced from `_get_agent_status` against `core.identities`) before falling back to `agent_metadata`. Two-pass council review at `docs/ontology/s21b-items-5-6-council-review-2026-04-30.md` — original CRITICAL #1 (PATH 0 auth bypass) + HIGH #2 (unguarded DB await) + post-fix HIGH #1 (test live-DB call) + MEDIUM #3 (PATH0 rejection test gap) all resolved; ontology debt + PATH0 pre-auth probe rate-limiting tracked in KG `2026-04-30T07:03:27.812659+00:00` + `2026-04-30T07:03:37.732240+00:00`.
  ...
  S8a-Ph2  ⚠️ 2026-04-30  three items in WIP, post-council revision — (a) `engaged_ephemeral` ScaleConstant + classify_agent branch (resolution-order: engaged_ephemeral before ephemeral; debt note: behavior cohort encoded in identity-class tag namespace, axis separation tracked separately); (c) promotion rule `total_updates ≥ 3` shipped as `class_promotion_sweeper_task` background task (30min cadence; single-CTE SQL with `FOR UPDATE SKIP LOCKED` + in-statement re-check; NULLIF cast guard against corrupted `total_updates`); (a-bis) Phase-1 stamp gap fixes — `_stamp_default_tags_on_onboard` body extracted to public `stamp_default_class_tags(meta=)` in `src/grounding/onboard_classifier.py` and wired into both `process_agent_update` auto-create sites in `src/mcp_handlers/updates/phases.py` (recovery branch passes `ctx.label` explicitly + warning-level log on stamp failure per council). New `stamp_untagged_identities` for the 72 currently-untagged backlog. 50 new tests (19 onboard + 14 indicator + 17 promotion live-DB incl. idempotent double-run + empty-string total_updates regression). Council pass: dialectic-knowledge-architect (9 findings — `session_like → engaged_ephemeral` rename, axis debt KG'd, ordering rationale documented), feature-dev:code-reviewer (4 findings — SQL CTE refactor + recovery branch label + NULLIF cast all addressed; pushed back on the jsonb_set "clobber" CRITICAL claim — under READ COMMITTED a second UPDATE re-reads the post-first-commit row, so the actual gap was SELECT-then-UPDATE staleness, fixed by the CTE), live-verifier (7 verified, flagged the 72-untagged vs 39-promotable population distinction → motivated `stamp_untagged_identities`). Decision (d) backfill via `scripts/migration/s8a_phase2_backfill.py --stamp-untagged && --promote --include-archived` after merge.
...
OPEN — actionable
  ...
  S20      PR1 merged (plugin#23); S20.1b merged (plugin#25) 2026-04-27; S20.1c/2/3/4/5/6 still open
  S21-b    items 5 + 6 follow-up — WIP-PR: unitares#245 (council-clean, awaiting merge); two-pass council review at `docs/ontology/s21b-items-5-6-council-review-2026-04-30.md`
...
BLOCKED — by upstream work
  ...
  R1       blocked on S8c data accumulation ≥1 week + S8a Phase 2 + R1 v3.2 council pass
  R2       blocked on R1
  R5       blocked on R2
...
**Critical path now (post-2026-04-29 sweep):** S21 closed end-to-end (S21-a 2026-04-28, S21-b 2026-04-29 across #231 + 8 follow-on commits hardening identity/KG/diagnostic-session/trust-tier surfaces). S1-a closed 2026-04-29 — six §12 items collapsed: onboard-handler refactor to shared helper, identity()/bind_session wiring, clock-skew tolerance (30s, bounded below TTL), 6 new regression tests including Chronicler-shape >1h-old token rejection. HTTP REST inherits transitively through `execute_http_tool` → handler dispatch (item 3 verified obsolete, no separate site needed). **NEW NEAR-TERM BLOCKER 2026-04-29 (process):** today's migration-drift parallel-PR collision (#236 + #237 closed via #238 + #239) added single-writer-surface guidance to CLAUDE.md/AGENTS.md; future code work in identity ontology + migration slots requires `gh pr list` preflight. **Remaining sequencing:** (a) S8a Phase 2 ratification once ≥1 week data accumulates (~2026-04-30 — opens tomorrow); (b) S1-b plugin/CLI helper migration to force_new posture; (c) R1 implementation row opens after S8c data ages + S8a Phase 2 — earliest mid-May. **R2 design can start in parallel with R1 wait-time.**
