...
OPEN — actionable
  ...
  R5       memory-deepening tooling — v0.1 KG cite-and-extend spec landed 2026-05-05; shadow scorer + synthetic tests + read-only diagnostic/batch sampler landed 2026-05-06, kept off the R2 hot path. Repeat live sample on 2026-05-06: 4 provisional pairs (3 insufficient parent memory, 1 absent) + 3 confirmed pairs (3 insufficient parent memory); durable audit table deferred until non-sparse integration evidence exists.
  ...
  S22      harness context provenance — write-context merge landed 2026-05-06 (`src/provenance_context.py`, KG `provenance.s22_context`, process-update `agent_state.provenance_context`); identity-response distinction landed 2026-05-06 (`identity_context` + top-level `identity_assurance`); durable state-json persistence + H5 comparable-entry diagnostic landed 2026-05-06 (`scripts/diagnostics/s22_h5_comparable_entries.py`), with live-schema, asyncpg row, target-key query compatibility, and missing-entry payload templates fixed the same day; current keyed live diagnostic reports one comparable Codex CLI entry for `s22-h5-2026-05-06`, so remaining closure needs matching Hermes and Claude Code entries
...
**Critical path now (2026-05-06 refresh):** S21 is closed end-to-end (S21-a 2026-04-28, S21-b via #231 + #245), S1-a/S1-b are shipped, S8a Phase 2 is shipped (#252), **S8b backfill is complete as of 2026-05-05**, **R2 Phase 1 shipped in #357 on 2026-05-05** (migration 036 + storage helpers, FSM/audit, cross-role pre-check, sweeper, process-update trigger), and **R2 Phase 1 telemetry now has a read-only diagnostic** (`scripts/diagnostics/r2_phase1_telemetry.py`) whose first live run defers Phase 2 (1.53 days observed, 3/50 confirmed pairs, 0/10 demoted pairs, 1/1 cross-role rejection). **R5 v0.1 KG cite-and-extend spec landed 2026-05-05 and its read-only shadow scorer/tests/diagnostic/batch sampler landed 2026-05-06**, with repeat live sampling still showing sparse parent KG memory rather than integration evidence. **S7 lineage-chain provenance v0.1 is closed as of 2026-05-06** with schema decision, persistence round-trip, authoritative chain builder, aggregation helper, S22 write-context merge, and an index-readiness/no-migration gate, **R6 episode-fork response-shape decision v2 is shipped 2026-05-02**, the `build_fork_context` call-site prerequisite is resolved by PR #284 (`eedf5203`), the `force_new` thread-position prerequisite is implemented as of 2026-05-05, and **R6 v2.1 onboard rich `thread_context` field shape is shipped 2026-05-05**. R1 has shipped through §4.3 public KG emission (#306/#309/#314/#320/#321/#324), with #341 deferring the R3 baseline consumer until a real reader exists, and an operator-facing 2026-05-05 maintenance sweep now covers provisional promotion, unsupported-lineage demotion via explicit `apply_orphans`, and public KG TTL archival; the first live sweep confirmed 1 of 3 provisional rows. **S22 has now shipped write-context provenance, identity-response distinction, durable state-json persistence, a read-only H5 comparable-entry diagnostic, missing-entry payload templates, and one live Codex CLI comparable entry for `s22-h5-2026-05-06`**; remaining S22 closure is matching Hermes and Claude Code entries. Remaining near-term work: (a) let R2 Phase 1 telemetry mature before opening Phase 2 consumers, (b) repeat R5 shadow sampling after the KG has more response-linked parent memory and defer durable audit history for now, (c) run the S22 H5 task across Hermes and Claude Code with the same `comparison_key`, using `scripts/diagnostics/s22_h5_comparable_entries.py --comparison-key s22-h5-2026-05-06 --show-missing-payloads` for payload shape, then verify completion with the same diagnostic, and (d) keep the `gh pr list` preflight discipline from the 2026-04-29 migration-drift collision before any new identity ontology or migration-slot work.
