...
- **D4 (next evidence source):** Before proposing any candidate-envelope promotion, create a fresh comparison key for the specific harness surface being tested (H7/H8/gateway/Discord/cron/Dispatch), run `r6_dogfood.py --assess` and `s22_candidate_envelope_coverage.py --json`, and record the observed field ratios here. Do not infer promotion readiness from date-default empty diagnostics or from historical H1/H3 rows.
