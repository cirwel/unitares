...
OPEN — actionable
  ...
  S2/S3    auto-resume and cross-channel token retirement are now unblocked by S1-c. Next action: audit remaining client/transport surfaces that still teach token-as-resume or cross-channel token carry-forward, then retire or rewrite them to `force_new + parent_agent_id` / explicit `client_session_id`.
