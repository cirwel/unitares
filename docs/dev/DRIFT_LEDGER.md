...
| `unitares/skills/` ↔ plugin `skills/` mirror | content divergence across repos | `sync-plugin-skills.sh` writes the fingerprint (`scripts/dev/skills_manifest.py`) into the mirror; the plugin CI recomputes it from the mirrored tree; full diff is `sync-plugin-skills.sh --check` | ⚠️ plugin CI catches a mirror edited without a re-sync; a mirror that is merely BEHIND canonical is caught only by `sync-plugin-skills.sh --check` (ship.sh, needs a local plugin checkout). The fingerprint is not committed in unitares (2026-09-24): it is derived data and its aggregate line made unrelated skill PRs conflict |
