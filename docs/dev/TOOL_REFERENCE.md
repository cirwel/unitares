...
Primary advisory model-help surface: send a brief, get back advisory model evidence, never a governed verdict — request_review produces that. effort='thorough' needs privacy='cloud_allowed'; against the default privacy='local' it refuses outright unless allow_degraded=true, which returns a standard local answer instead. effort='thorough' asks a strong model from a different family than the caller's when the caller's family can be detected (Claude, Codex or Antigravity, whichever the operator has available); an undetected caller may get any of them. Requires a bound identity. Each routed consultation is audited as event_type='consultation', readable by any bound agent: route, outcome, and HMAC hashes of the brief and advice under a key returned only to you in record.hash_key, never their text. Use call_model or delegate_inference only for explicit provider, host, model or timeout control.
