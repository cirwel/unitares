...
|-------|-------------------|-------|
| Public product sentence | The tagline is the README's own heading, "Accountability infrastructure for long-running AI agents." The definition is the three sentences of the README's opening body paragraph, beginning "UNITARES is self-hosted accountability infrastructure for operators running multiple AI agents." — not the three short lines under the heading. "Many agents, one record." is a display headline for the site, not a definition. Every public surface copies these verbatim rather than re-deriving them. Retired, in any hyphenation and whether the noun is "AI agents" or "AI-agent fleets": "Runtime governance for heterogeneous AI-agent fleets", "Infrastructure for long-lived AI agents", "Runtime governance for long-lived AI agents", "runtime state telemetry for long-lived AI agents". Non-markdown surfaces are covered by `_CONTESTED_EXTRA_SURFACES`; the GitHub repository description is covered by `check_repo_description.py`, run weekly by the `Positioning Surfaces` workflow and pinned as wired by `tests/test_public_positioning_contract.py`. Only the social-preview PNG is still checked by hand, because the tagline is pixels there — which is why the standing rule is not to bake a sentence into an image. | `README.md` (owner) · `scripts/diagnostics/check_doc_drift.py` (`CANONICAL_TAGLINE` declares the string; `PUBLIC_POSITIONING_CHECKS` pins it on README, `CITATION.cff` and the Pages landing; `SINGLE_OPERATOR_SCOPE` pins "single-operator" on the five headline surfaces) · `scripts/diagnostics/check_doc_health.py` (`_CONTESTED_CLAIMS` bans the retired wordings, `_CONTESTED_EXTRA_SURFACES` reaches non-`.md` files) · `scripts/diagnostics/check_repo_description.py` (the GitHub description; skips when it cannot reach GitHub) · `tests/test_readme_evidence_contract.py`, `tests/test_build_public_site.py` · `scripts/dev/brand/render_social_preview.py` (`TAGLINE`) · `cirwel/cirwel-site` `EDITORIAL.md` §1. Note `PRODUCT_CATEGORY` is a *separate* claim from `CANONICAL_TAGLINE`: it pins the product noun ("federation kernel"), not this sentence. Since 2026-09-25 each headline definition on the five pinned surfaces (`README.md`, `docs/PRODUCT_DEFINITION.md`, `docs/CAPABILITIES_AND_DEPLOYMENT.md`, `src/tool_modes.py`, `pyproject.toml`) plus `docs/deployment/glama.md` carries the noun's scope, "single-operator federation kernel"; this qualifies the noun adopted in #2159 and does not supersede it. The multi-authority sense is written "cross-operator" or "multi-principal" in the docs #2451 touched and, since the #2454 follow-up, in `docs/EVIDENCE_AND_LIMITS.md`, `docs/SCOPE_AND_THREAT_MODEL.md`, the `drr.v1` packet and the dormant-capability registry. Frozen and historical documents keep their wording, as do the uses #2451 left on purpose because they are tied to code identifiers or are not clearly multi-authority. This wording qualification is an agent-authored change under the operator's 2026-09-25 delegation to proceed best for the federation's future; unlike the D3a/D3b/D4/D5 decisions in `docs/proposals/active/federation-trust-decisions-2026-09-25.md`, whose Basis paragraph this text does not extend to, no adversarial design review of this specific wording is recorded, and the operator's ratification is the merge (#2451). |
| Tool surface measurement | Catalog fingerprints describe source schemas, and the `/mcp/` registrar advertises those schemas verbatim (`tests/test_mcp_schema_parity.py`). Measure the final MCP listing for payload costs. Static hint candidates do not establish caller severity or observed failure incidence. | `src/tool_mode_listing.py` · `scripts/diagnostics/tool_surface_cost.py` · `scripts/diagnostics/hint_target_advertisement.py` |
| Redis posture | Redis is the de-facto primary session store; the server boots without it in a degraded local-only mode (fine for the demo; sessions won't persist). It is not "optional" in production. | `docs/UNIFIED_ARCHITECTURE.md` · `docs/proposals/archive/redis-retirement-v0.md` |
...
| SDK publication | `unitares-sdk` 0.1.0 is published on PyPI. Use the matching Git tag only for deliberate unreleased-source testing. | `agents/sdk/README.md` · `docs/COMPATIBILITY.md` |
...
| CIRS per-protocol actions | `cirs_protocol` routes `action` per protocol, and only `void_alert` and `state_announce` take `emit`. `coherence_report` takes `compute` (pairwise, `target_agent_id` required) or `query`; `boundary_contract` takes `set`/`get`/`list`; `governance_action` takes `initiate`/`respond`/`query`/`status`. Neither `coherence_report` nor `governance_action` has an `emit` action. | `src/mcp_handlers/cirs/coherence.py` · `src/mcp_handlers/cirs/governance_action.py` · `docs/guides/CIRS_PROTOCOL.md` |
| CIRS boundary_contract parameters | `boundary_contract set` sets the caller's own contract and takes `trust_default`, `trust_overrides`, `void_response_policy`, `max_delegation_complexity` and `accept_coherence_threshold`, all optional; it takes no `target_agent_id`. `trust_level` and `void_policy` are read by no handler, so a call using them stores `partial` and `notify` whatever it asked for. | `src/mcp_handlers/cirs/boundary.py` · `src/mcp_handlers/schemas/core.py` · `docs/guides/CIRS_PROTOCOL.md` |
...
| Current public ablation read | The frozen 2026-08-09 trusted-anchor matrix classifies all 12 overall scope/window/lead slices `NON_DETECTION` against the best-of-candidates null (selective p = 0.070–0.567). That does not establish absence or refutation; unadjusted lift is not selection-adjusted evidence, and no prevention is demonstrated. | `docs/operations/eisv-ablation-frozen-2026-08-09.md` · `docs/REVIEWER_GUIDE.md` |
| What that read licenses | A non-detection, not a demonstrated negative and not a ceiling on forecasting power. Predictive lift is **unresolved** pending the pre-registered 2026-12-01 read. The instrument's power on a cohort of that shape is measured, not assumed. | `docs/operations/falsifiability-power-audit-2026-08-23.md` · `scripts/analysis/ablation_power_probe.py` · `docs/proposals/registered/eisv-outcome-grounding-stop-rule-v0.md` |
