...
|-------|-------------------|-------|
| Public product sentence | The tagline is the README's own heading, "Accountability infrastructure for long-running AI agents."; the definition is the README's first three sentences; "Many agents, one record." is a display headline for the site, not a definition. Every public surface copies these verbatim rather than re-deriving them. Retired: "Runtime governance for heterogeneous AI-agent fleets", "Infrastructure for long-lived AI agents", "Runtime governance for long-lived AI agents", "runtime state telemetry for long-lived AI agents". Two surfaces are not files and are checked by hand: the GitHub repository description and the social-preview PNG. | `README.md` (owner) · `scripts/diagnostics/check_doc_drift.py` (`PUBLIC_POSITIONING_CHECKS`) · `scripts/dev/brand/render_social_preview.py` (`TAGLINE`) · `cirwel/cirwel-site` `EDITORIAL.md` §1 |
