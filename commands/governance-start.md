...
`start_session(...)` is an equivalent alias (same parameters, same rules). Invoking the alias returns the normalized agent-experience envelope — `next_action`/`state_summary` first, with `agent_uuid`, `client_session_id`, `agent_id`, `display_name`, `is_new`, `identity_resolution_outcome` and a compact `identity_assurance` (tier, session_source, caller_proven, baseline) lifted to the top level. For a plain fresh mint (`response_shape: "routine"`) the canonical payload is not repeated beneath them. That includes a mint on a thread earlier sessions occupied, where `state_summary` carries `episode_fork_kind: "sibling_locus"` and the earlier node's `predecessor_uuid` (co-location is not lineage; do not declare it as a parent), and a named mint, whose `resident_registration` verdict is lifted in compact form (`status`, `on_roster`, and for `not_on_roster` (and, briefly, `caller_supplied_tags`) a short `detail`: what it costs, that this identity cannot gain the tags, and the fix: add the name to the roster, restart, mint fresh). Any other mint (a resume miss, a reactivated identity, a declared lineage, a spawn reason other than `new_session`, a sibling whose earlier nodes were pruned, a label rename, an unrecognised notice), and any call with `response_mode="full"`, also carries it under `raw_governance`; for a mint that was not plain, `response_shape_reason` names the field that kept it. Read the uuid from `agent_uuid`. The canonical payload defaults to `response_mode="minimal"`, which omits `session_resolution_source` and `continuity_token_supported`; pass `response_mode="full"` when you need `session_resolution_source` for the cache below.
