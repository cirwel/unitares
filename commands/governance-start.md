...
`start_session(...)` is an equivalent alias (same parameters, same rules). Invoking the alias returns the normalized agent-experience envelope — `next_action`/`state_summary` first, with `agent_uuid`, `client_session_id`, `agent_id`, `display_name` and any mint-time warning (`resident_registration`, `label_renamed`, `bootstrap`) at the top level. By default it does not repeat the canonical payload under `raw_governance`; that payload defaults to `response_mode="minimal"` and omits `session_resolution_source` and `continuity_token_supported` in any case. When you need `session_resolution_source` for the cache below, pass `response_mode="full"` on this one onboarding call, which returns the complete payload under `raw_governance`. Decide that before calling: a second `start_session(force_new=true)` mints a second identity. The raw `onboard(...)` returns its payload directly.
