...
handlers and their keys are not declared. The key inventory is read from the
handlers' own source rather than hand-listed, so a handler that starts reading
a new argument fails here the day it does.
"""
...
def _keys_read(handler) -> dict[str, list]:
    ...
    per-action helper (``_handle_void_alert_query``), and two handlers sharing
    a module need not read the same keys or share defaults. So walk from the
    handler's function definition into the module-level functions it calls,
    transitively."""
...
def test_the_scan_sees_the_handlers():
    """Guard the inventory: an empty scan would pass every test below.
