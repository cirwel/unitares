...
def test_dated_review_record_retains_frozen_contract(
    tmp_path, monkeypatch, doc_health
):
    """A reasoned sidecar review can retain a frozen doc without editing it."""
    review = (
        tmp_path
        / "docs"
        / "proposals"
    ...
    review.write_text(
        "| [contract](../frozen.md) | Retain current contract | "
        "The live gate remains open and this document defines its safety envelope. |\n"
    )
    warnings = _demotion_for(
        tmp_path,
        monkeypatch,
...
def test_dated_review_record_requires_substantive_reason(
    tmp_path, monkeypatch, doc_health
):
    """A sidecar table cannot silence a candidate with an empty rationale."""
    review = (
        tmp_path
        / "docs"
        / "proposals"
    ...
    review.parent.mkdir(parents=True)
    review.write_text("| [contract](../frozen.md) | Retain | too short |\n")
    warnings = _demotion_for(
        tmp_path,
        monkeypatch,
        doc_health,
