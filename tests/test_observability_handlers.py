...
# ---------------------------------------------------------------------------
    ...
    @pytest.mark.asyncio
    async def test_window_bounds_correct_across_mixed_utc_offsets(self):
        """The DB hands back datetimes; combining group bounds by ISO-string
        comparison picks the wrong one whenever offsets differ. These two
        instants are ordered OPPOSITE to their string forms."""
        from datetime import datetime, timedelta, timezone

        # 01:00 UTC
        early = datetime(2026, 5, 20, 1, 0, tzinfo=timezone.utc)
        # 20:00-06:00 == 02:00 UTC the next day: later instant, earlier string
        late = datetime(2026, 5, 19, 20, 0, tzinfo=timezone(timedelta(hours=-6)))

        assert early < late                              # chronologically
        assert early.isoformat() > late.isoformat()      # ...but not as strings

        async def fake_query(**kwargs):
            return [self._fake_event("2026-05-19T10:00:00+00:00", "agent-a")]

        async def fake_aggregate(**kwargs):
            return [
                {"agent_id": "agent-a", "event_type": "e", "count": 1,
                 "first_ts": early, "last_ts": early},
                {"agent_id": "agent-b", "event_type": "e", "count": 1,
                 "first_ts": late, "last_ts": late},
            ]
        ...
        with patch("src.audit_db.query_audit_events_async", new=fake_query), \
            ...
            from src.mcp_handlers.observability.handlers import handle_audit_events
            result = await handle_audit_events({
                "event_type": "e", "since": "7d", "limit": 1,
            })

        data = parse_result(result)
        # String comparison would invert both of these.
        assert data["first_ts"] == early.isoformat()
        assert data["last_ts"] == late.isoformat()
        assert data["total_emits"] == 2
