...
def test_frozen_cohort_contract_keeps_the_registered_fixture_rule():
    """2026-09-02: the shared fixture default moved to `corrected`; the cohort
    contract froze its predicate before rules existed, so this script pins the
    registered reading and a scraped-only row stays a fixture here until a
    CONTRACT_VERSION bump adopts the corrected rule."""
    live = _row(0)
    scraped = OutcomeRow(
        **{
            **_row(1).__dict__,
