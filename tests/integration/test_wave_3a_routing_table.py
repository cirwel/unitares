...
Specification:
    ``docs/proposals/archive/beam-wave-3a-read-only-handlers.md`` v0.2 §3.1
...
async def test_routing_table_miss_passthrough(
    ...
    routing-table integration in ``tool_registration.get_tool_wrapper`` were
    deleted. Tests must observe the wrapper, not the routing-table state.
    """
    ...
    # which path fired. ``patch("src.tool_registration.dispatch_tool")`` works
    # because the wrapper closes over the module-level binding in
    # tool_registration (FIND-A3 hoisted ``_wave3a_get_route`` /
    # ``_wave3a_proxy_to_beam`` similarly).
    ...
    monkeypatch.setattr(
        tool_registration, "_wave3a_proxy_to_beam", fake_proxy
    )
    ...
    wrapper = tool_registration.get_tool_wrapper("_wave3a_test_miss_tool")
    ...
    tool_registration._tool_wrappers_cache.pop("_wave3a_test_miss_tool", None)
