...
| **UNITARES Resident** | A first-party, general-purpose agent userland built entirely on the public interfaces above. | Its separate repository ships an early runtime skeleton, not yet a usable general-purpose agent. It must not receive a privileged scoring or database path. |
