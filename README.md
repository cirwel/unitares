...
| Benefit from review and coordination | **Untested** | No governed-versus-ungoverned comparison has been run, so no outcome improvement is established, and the sparse record here is not that null either. Coverage is also narrower than the mechanism: these records cover dialectic-mediated review, and review conducted through subagent councils or external models leaves no row here unless filed through `reviewer_provenance`. A low count therefore does not separate a mechanism that was never surfaced, never reachable, or never recorded from one that was recorded and genuinely unused; nothing here rules out the first three. |
