...
| `knowledge` | Opens individual shared-memory results with `action="details"`. |
| `describe_tool` | Expands abridged parameter documentation on demand. |
| `health_check` | Inspects server health when recovery guidance calls for it. |
...
`lite` adds the remaining consolidated routers and `list_tools`, and `full`
advertises every registered tool. See [Beyond the fourteen](#beyond-the-fourteen).
...
The fourteen-tool `standard` profile requires v2.23.0 or later. v2.22.0 defaults
...
the remaining consolidated routers and `list_tools` in addition to the fourteen;
...
| Expand a tool contract or inspect server health | `describe_tool(tool_name=...)`, `health_check()` | `standard`, `lite`, `full` |
| Open a shared-memory result | `knowledge(action="details", discovery_id=...)` | `standard`, `lite`, `full` |
| Enumerate the live surface | `list_tools()` | `lite`, `full` |
| Reach an action the workflow names do not cover | the other routers: `dialectic`, `observe`, `agent`, ... | `lite`, `full` |
...
| | |
|---|---|
| **42 tools** on the wire | fourteen advertised by default and the rest behind `GOVERNANCE_TOOL_MODE`; 8 of the 42 are consolidated routers over 52 actions, 8 workflow aliases carry the agent-facing names, and a 70-entry alias table resolves legacy names |
