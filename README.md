...
| Benefit from review and coordination | **Untested** | No governed-versus-ungoverned comparison has shown an outcome improved by review. Coverage is also narrower than the mechanism: these records cover dialectic-mediated review, and review conducted through subagent councils or external models leaves no row here unless filed through `reviewer_provenance` — a path **no message has ever used** (measured 2026-08-28: of 672 messages, 40 carry a `reviewer_backend` stamp, 37 of those record no kind, 3 are `agent_submitted`, **0 are `external_consult`**). For that channel the zero means *not recorded*, not *no effect* — the third of the four states the measurement-authority rule in `CLAUDE.md` separates, and the reason this row cannot move on outside review however much of it happens. The cheapest evidence needing no control group is the escape rate: defects a review pass surfaces that the test suite did not, countable once passes are filed. |
