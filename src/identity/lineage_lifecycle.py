...
- Always stamps `lineage_last_eval_at` after R1 runs (and after a
  grace short-circuit). No stamp on cadence skip / no-parent skip.
- Emits one of five audit events per terminal transition:
  ``lineage_promoted``, ``lineage_demoted``, ``lineage_grace_expired``.
...
class LineageEvalOutcome:
    ...
    - ``skipped_reason`` is one of ``"no_parent"``, ``"within_cadence"``
      when the FSM declined to evaluate; otherwise ``None``.
    """
...
async def evaluate_lineage_for(
    ...
    *,
    ...
    # 2. Cadence guard — if we evaluated this row recently, skip.
    ...
    # 3. Grace expiration short-circuit — only meaningful for
    ...
    # 4. Score the lineage with R1.
    score = await score_trajectory_continuity(
        parent_id, successor_id, min_observations=min_observations,
    )
    # Cadence stamp goes after every R1 invocation regardless of the
    # outcome — otherwise an inconclusive verdict would leave
    # `lineage_last_eval_at` stale and the next sweeper tick would
    # immediately re-run R1.
    await backend.stamp_lineage_eval(successor_id)
