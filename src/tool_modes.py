...
Writes are attributed. Fresh processes get fresh identities; real lineage records inherited work, not authority or cross-process sameness, and it exists only when declared, never inferred from a shared thread. Bind once with start_session(force_new=true) and keep the returned client_session_id; pass it on every later call so writes are attributable. sync_state submits a work report for state derivation and returns the state estimate, a policy action (proceed, guide, or pause), and a named reason; the original report text is not retained as durable history. proceed and guide are advisory. An applied pause is a hard stop: governed writes hold until self_recovery lifts it, a dialectic review (request_review) resolves it, the agent is resumed (agent(action='resume'), operator tools, or an automatic safety net), or it expires. self_recovery refuses while risk stays high, and review records your written reflection in shared memory. record_result grades a check-in against a real outcome — without outcomes the estimate is self-report. check_working_state reads the verdict without writing.
...
def should_include_tool(
    tool_name: str, mode: str = "full", client_type: str = None
) -> bool:
    """
