...
Writes are attributed. Fresh processes get fresh identities; real lineage records inherited work, not authority or cross-process sameness, and it exists only when declared, never inferred from a shared thread. Bind once with start_session(force_new=true) and keep the returned client_session_id; pass it on every later call so writes are attributable. sync_state submits a work report for state derivation and returns the state estimate, a policy action (proceed, guide, or pause), and a named reason; the original report text is not retained as durable history. A pause refuses further governed writes until self_recovery accepts a real reflection or the pause expires; proceed and guide are advisory. record_result grades a check-in against a real outcome — without outcomes the estimate is self-report. check_working_state reads the verdict without writing.
