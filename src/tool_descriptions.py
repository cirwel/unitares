...
_IDENTITY_DESCRIPTION_OVERRIDES = {
    ...
    "identity": (
        ...
        "display name. Not a plain read: a call carrying no proof argument "
        "at all is gated to a fresh mint, so it persists a new agent and "
        "reports on that one, marked caller_proven=false. A call carrying "
        "only a cosmetic name= skips that gate and can instead infer a "
        "co-located binding — pass client_session_id to get your own back. name= persists a "
        ...
        "identity(client_session_id='...') to see the identity currently "
        "bound to your session; an argument-less call cannot answer that "
        "question, because it mints before it reads. Use "
...
_INFERENCE_DESCRIPTION_OVERRIDES = {
    ...
    "delegate_inference": (
        ...
        "tools, a read-only sandbox, or plan mode in an empty workspace: it "
        "answers, it cannot change anything. It requires a "
        ...
        "with the host's authenticated CLI on PATH; "
        "UNITARES_HOST_ADAPTER_DISABLED_HOSTS switches single hosts off. On "
    ...
    "consult": (
        ...
        "Each routed consultation is audited as event_type='consultation', "
        "readable by any bound agent: outcome, the route when a well-formed "
        "result came back, and HMAC hashes of the "
        "brief and advice under a key returned only to you in record.hash_key, "
        "never their text. Use call_model or "
    ...
    "dialectic": (
        ...
        "backend (local, codex, claude, antigravity, or external, alias "
        "gemini); a "
