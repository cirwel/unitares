    ...
    return [
        ...
        InferenceHost(
            ...
            notes=(
                ...
                "Codex served ASYNC via the agent-orchestrator, not the sync "
                "call_model path. Requires "
                ...
                "AGENT_ORCHESTRATOR_BEARER_TOKEN. The model-aware app-server "
                "transport reports the selected/effective model, provider, "
                "service tier, per-turn usage, and explicit reroutes. Set "
                "UNITARES_CODEX_APP_SERVER_INSTRUMENTATION=0 to use the older "
                "exec JSONL path, which intentionally reports the model as "
                "unavailable. Separately, the dialectic reviewer "
