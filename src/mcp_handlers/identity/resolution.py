...
def _resolve_miss_audit_admit(
    ...
    ``(session_key, reason, suppressed, fields)`` for OTHER keys whose window
    has closed with a pending count, or that were evicted with one.
    """
    ...
    flushes = []
    for other, (written_at, pending, other_fields) in list(_resolve_miss_audit_state.items()):
        if other == key or now - written_at < _RESOLVE_MISS_AUDIT_WINDOW_SECONDS:
            continue
    ...
    if state is None and len(_resolve_miss_audit_state) >= _RESOLVE_MISS_AUDIT_MAX_KEYS:
        ...
        _, pending, oldest_fields = _resolve_miss_audit_state.pop(oldest)
        if pending:
            flushes.append((oldest[0], oldest[1], pending, oldest_fields))
    _resolve_miss_audit_state[key] = [now, 0, dict(fields or {})]
...
def _audit_session_resolve_miss(
    *,
    ...
    for f_key, f_reason, f_pending, f_fields in flushes:
        try:
