...
# also carries predecessor_uuid, episode_fork_kind and the ~230 B sentence
...
# paid for on top of its class budget; the largest measured is ~230 B.
...
def _thread_context_is_routine(
    thread_context: Dict[str, Any], payload: Dict[str, Any]
) -> bool:
    """The mint's thread position says nothing the envelope does not carry.
    ...
    """
    if thread_context.get("identity_lineage_fork") is not False:
        return False
    kind = thread_context.get("episode_fork_kind")
    if kind == "none":
        return not (thread_context.get("predecessor") or thread_context.get("is_fork"))
    if kind == "sibling_locus":
        return (
            thread_context.get("spawn_reason") not in LINEAGE_SPAWN_REASONS
            and payload.get("lineage_state") == "no_lineage_declared"
            and not payload.get("provisional_lineage")
        )
    return False
...
def _routine_mint_blockers(payload: Dict[str, Any]) -> List[str]:
    ...
    if isinstance(thread_context, dict) and not _thread_context_is_routine(
        thread_context, payload
    ):
        kind = thread_context.get("episode_fork_kind") or "missing"
        blockers.append(f"thread_context.episode_fork_kind={kind}")
