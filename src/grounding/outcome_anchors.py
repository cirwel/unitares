...
#   before that key existed, and rows the Phase-5 writers insert directly
#   (never stamped, so never excluded by the flag), are classified by
#   ``prediction_source``. Calibration still never trains on
#   an excluded row: this rule changes what the validation instruments count,
#   not what calibration learns from. Opt-in (`--fixture-rule corrected`):
#   the shared default stays `registered`, so no instrument's output moves on a
#   deploy and the pre-registered read keeps the predicate it was registered
#   with; the packet's Selection block records why the default was not flipped.
