    ...
    *,
    ...
    return {
        ...
        "operational": {
            ...
            "hook_parent_process_alive": process["hook_parent_process_alive"],
    ...
    *,
    ...
    for event in events:
        ...
        process = processes.setdefault(
            key,
            {
                ...
                "hook_parent_process_alive": False,
        ...
        if kind == "heartbeat" and (
            ...
            process["hook_parent_process_alive"] = (
    ...
    for process in processes.values():
        ...
        tool_activity_after_report = bool(
            last_tool_activity
            and (last_reflection is None or last_tool_activity > last_reflection)
        )
        host_observation_after_report = bool(
            last_reflection is None or last_host_observation > last_reflection
        )
        ...
        rows.append(
            {
                **process,
                ...
                "last_operational_at": _iso(last_host_observation),
                "operational_age_seconds": host_observation_age,
                "operational_recent": bool(
                    tool_activity_age is not None
                    and tool_activity_age <= RUNTIME_RECENT_SECONDS
                ),
                "host_process_alive": bool(
                    process["hook_parent_process_alive"] and heartbeat_recent
                ),
    ...
    last_host_observation = max(
        (_as_utc(row["last_host_observation_at"]) for row in rows), default=None
    )
    last_reflection = max(
