defmodule UnitaresLeasePlane.BuildInfo do
  ...
  @doc """
  The boot sha cached by `resolve!/0`, or `"unknown"`.

  ⛔NEVER resolves on demand, and the difference is the whole point. Hot code
  loading does not re-run `Application.start/2`, so on a node that booted BEFORE
  this module existed the cache is empty when the freshly reloaded router first
  asks. An on-demand fallback would then run `git rev-parse` and cache the sha
  of the CURRENT checkout as this node's "boot" sha — and the status row would
  read CURRENT while the supervision tree was still the pre-reload one. That is
  precisely the false-negative this value exists to prevent, reintroduced by the
  convenience of a fallback.

  `"unknown"` is the honest answer there: it renders as HOT-RELOAD(?) and asks
  for a restart, which is what actually fixes it. Tests seed the cache by
  calling `resolve!/0` (see test_helper.exs).
  """
