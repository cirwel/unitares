defmodule UnitaresSentinel.LeaseStarvationTest do
  ...
  immortal-lease incident: `resident:/sentinel_cycle` refused on 1,742 of 2,012
  tick attempts across four episodes, zero alerts, every liveness signal healthy).
  ...
  test "a non-alerting episode retries an older debt once per granted tick, not twice" do
    # `owe_closure(tracker, nil)` is a pass-through, so piping
    # `retry_pending_closure/2` both before and after it fired the SAME debt
    # twice in one granted tick whenever this episode never alerted — the normal
    # case. That double-POSTs an identical finding and burns two of five
    # attempts at once, collapsing the retry window from 5 granted ticks to 3
    # under exactly the blip-interleaved traffic the debt exists to survive, and
    # doubling load on the gov-MCP that was already down.
    parent = self()
    ...
    tracker =
      tracker(state_path: tmp_state_path())
      |> block(held_by_other_scope(), 0)
      |> LeaseStarvation.maybe_emit(
        emit_opts(fn _u, _b, _h, _t -> {:ok, 200, ~s({"success":true})} end, now: at(60))
      )
    ...
    # A sub-threshold blip opens a non-alerting episode 2 on top of the debt.
    tracker = block(tracker, unavailable_scope(), 150)
    assert tracker.lease_blocked_last_emitted_multiple == 0

    # One granted tick must cost the debt exactly ONE attempt.
    tracker = LeaseStarvation.clear(tracker, emit_opts(failing_post, now: at(180)))

    assert_receive :closure_attempted
    refute_receive :closure_attempted, 0
    assert tracker.lease_blocked_pending_closure.attempts == 2
  end
